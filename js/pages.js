...
/* ==========================================================================
   H. MODUL UJIAN & KARTU QR (v2.1)
   --------------------------------------------------------------------------
   Berisi enam halaman:
     dosen-bank-soal        — menyusun & mengelola bank soal
     dosen-ujian            — daftar ujian + pembuatan ujian
     dosen-ujian-hasil      — hasil, koreksi uraian, pengawasan, buka blokir
     siswa-ujian            — daftar ujian yang tersedia bagi siswa
     siswa-ujian-kerjakan   — halaman pengerjaan dengan timer & pengawasan
     kartu-qr               — kartu QR siswa + pengaturan metode absensi

   Catatan arsitektur: seluruh data ujian diambil sesuai kebutuhan (bukan lewat
   getInitialAppData), karena isinya bisa sangat besar dan hanya relevan saat
   halaman terkait dibuka. Muatan awal aplikasi tetap ringan.
   ========================================================================== */

/** Penyimpanan sementara khusus modul ujian, di luar DB agar tidak ikut ter-refresh. */
const UJIAN_STATE = {
  bankSoal: [],
  daftarUjian: [],
  hasil: null,
  /* Sesi pengerjaan yang sedang berjalan */
  sesi: null,
  timer: null,
  pengawas: null,
  jawabanTertunda: {},
...
PAGES['dosen-bank-soal'] = function () {
  return headerHalaman('Bank Soal',
    'Soal disimpan terpisah dari ujian, sehingga dapat dipakai ulang pada semester berikutnya.',
    '<button class="btn btn-outline" onclick="formImporSoal()"><i data-icon="upload"></i><span>Impor Excel</span></button>' +
    '<button class="btn btn-primary" onclick="formSoal()"><i data-icon="plus"></i><span>Tambah Soal</span></button>') +
    '<section class="card">' +
    '<div class="card-head"><i data-icon="library"></i><h2>Daftar Soal</h2>' +
    '<div class="head-tools"><select class="select" id="filterMapelSoal" onchange="muatBankSoal()" style="min-width:200px">' +
    '<option value="">Semua mata pelajaran</option>' +
    opsiSelect(mapelDiampu(), 'ID', function (x) { return x.Kode + ' — ' + x.Nama; }) +
    '</select></div></div>' +
    '<div id="areaBankSoal"><div class="card-body"><div class="skeleton-list">' +
    '<div class="skeleton-row"></div><div class="skeleton-row"></div><div class="skeleton-row"></div>' +
    '</div></div></div></section>';
};

PAGE_INIT['dosen-bank-soal'] = function () { muatBankSoal(); };

function muatBankSoal() {
  const mapelId = ($('#filterMapelSoal') || {}).value || '';
  api('apiGetBankSoal', AppState.sessionToken, mapelId).then(function (res) {
    const area = $('#areaBankSoal');
    if (!area) return;
    if (!res.success) {
      area.innerHTML = '<div class="card-body">' +
        kosongState('alert-circle', 'Gagal memuat bank soal', res.message) + '</div>';
      return;
    }
    UJIAN_STATE.bankSoal = res.data || [];
    gambarBankSoal();
  }).catch(function (e) {
    const area = $('#areaBankSoal');
    if (area) area.innerHTML = '<div class="card-body">' + kosongState('alert-circle', 'Gagal memuat', e.message) + '</div>';
  });
}

function gambarBankSoal() {
  const area = $('#areaBankSoal');
  if (!area) return;
  const data = UJIAN_STATE.bankSoal;

  if (!data.length) {
    area.innerHTML = '<div class="card-body">' +
      kosongState('library', 'Bank soal masih kosong',
        'Tambahkan soal satu per satu, atau impor sekaligus dari berkas Excel.',
        '<button class="btn btn-primary" onclick="formSoal()">' + svgIcon('plus', 18) + ' Tambah Soal Pertama</button>') +
      '</div>';
    return;
  }

  const ringkas = { PG: 0, Uraian: 0, Upload: 0 };
  data.forEach(function (s) { ringkas[s.Jenis] = (ringkas[s.Jenis] || 0) + 1; });

  area.innerHTML =
    '<div class="card-body" style="padding-bottom:0"><div class="row" style="margin-bottom:12px">' +
      '<span class="badge badge-info plain">Pilihan Ganda: <b>' + ringkas.PG + '</b></span>' +
      '<span class="badge badge-neutral plain">Uraian: <b>' + ringkas.Uraian + '</b></span>' +
      '<span class="badge badge-neutral plain">Unggah Berkas: <b>' + ringkas.Upload + '</b></span>' +
    '</div></div>' +
    tabelGenerik({
      id: 'tbl_bank_soal', data: data, perPage: 12, kosong: 'Tidak ada soal.',
      kolom: [
        { key: 'Jenis', label: 'Jenis', render: function (r) { return badgeJenisSoal(r.Jenis); } },
        { key: 'Pertanyaan', label: 'Pertanyaan', render: function (r) {
            const t = String(r.Pertanyaan || '');
            return '<div style="max-width:460px">' + esc(t.length > 130 ? t.slice(0, 130) + '…' : t) + '</div>';
          } },
        { key: 'MapelID', label: 'Mata Pelajaran', render: function (r) { return esc(namaMapel(r.MapelID)); } },
        { key: 'Tingkat', label: 'Tingkat', render: function (r) {
            const warna = { Mudah: 'badge-success', Sedang: 'badge-info', Sulit: 'badge-error' };
            return '<span class="badge ' + (warna[r.Tingkat] || 'badge-neutral') + ' plain">' + esc(r.Tingkat || '-') + '</span>';
          } },
        { key: 'Bobot', label: 'Bobot', cls: 'num' },
        { key: 'aksi', label: '', render: function (r) {
            return '<div class="row-actions">' +
              '<button class="btn btn-ghost btn-sm" title="Ubah" onclick="formSoal(\'' + esc(r.ID) + '\')">' + svgIcon('edit', 16) + '</button>' +
              '<button class="btn btn-ghost btn-sm danger" title="Hapus" onclick="hapusSoal(\'' + esc(r.ID) + '\')">' + svgIcon('trash', 16) + '</button>' +
              '</div>';
          } }
      ]
    });
  renderIcons(area);
  gambarTabel('tbl_bank_soal');
}

function badgeJenisSoal(jenis) {
  const peta = {
    PG:     ['badge-info',    'Pilihan Ganda'],
    Uraian: ['badge-neutral', 'Uraian'],
    Upload: ['badge-warn',    'Unggah Berkas']
  };
  const p = peta[jenis] || ['badge-neutral', jenis || '?'];
  return '<span class="badge ' + p[0] + ' plain">' + esc(p[1]) + '</span>';
}

/** Formulir tambah/ubah soal. Bagian opsi hanya muncul untuk pilihan ganda. */
function formSoal(id) {
  const d = id ? (UJIAN_STATE.bankSoal.filter(function (s) { return s.ID === id; })[0] || {}) : {};
  const opsi = (d.Opsi && d.Opsi.length) ? d.Opsi : ['', '', '', ''];

  openModal({
    title: id ? 'Ubah Soal' : 'Tambah Soal', size: 'wide',
    body: '<form id="formSoal" onsubmit="return false"><div class="form-grid">' +
      '<div class="field"><label class="label">Jenis Soal <span class="req">*</span></label>' +
      '<select class="select" name="Jenis" id="jenisSoalInput" onchange="ubahJenisSoal()" required>' +
      '<option value="PG"' + (d.Jenis === 'PG' || !d.Jenis ? ' selected' : '') + '>Pilihan Ganda</option>' +
      '<option value="Uraian"' + (d.Jenis === 'Uraian' ? ' selected' : '') + '>Uraian</option>' +
      '<option value="Upload"' + (d.Jenis === 'Upload' ? ' selected' : '') + '>Unggah Berkas</option>' +
      '</select></div>' +

      '<div class="field"><label class="label">Mata Pelajaran <span class="req">*</span></label>' +
      '<select class="select" name="MapelID" required>' +
      opsiSelect(mapelDiampu(), 'ID', function (x) { return x.Kode + ' — ' + x.Nama; }, d.MapelID, '— Pilih —') +
      '</select></div>' +

      '<div class="field full"><label class="label">Pertanyaan <span class="req">*</span></label>' +
      '<textarea class="textarea" name="Pertanyaan" rows="3" required ' +
      'placeholder="Tulis pertanyaan selengkapnya di sini.">' + esc(d.Pertanyaan || '') + '</textarea></div>' +

      '<div class="field full" id="areaOpsiPG"' + (d.Jenis && d.Jenis !== 'PG' ? ' hidden' : '') + '>' +
      '<label class="label">Pilihan Jawaban <span class="req">*</span></label>' +
      '<p class="hint" style="margin:0 0 8px">Tandai lingkaran di sebelah kiri opsi yang benar. Opsi yang dikosongkan tidak akan ditampilkan ke siswa.</p>' +
      '<div id="daftarOpsi">' + opsi.map(function (o, i) { return barisOpsi(i, o, Number(d.KunciJawaban) === i); }).join('') + '</div>' +
      '<button type="button" class="btn btn-outline btn-sm" style="margin-top:8px" onclick="tambahOpsi()">' +
      svgIcon('plus', 15) + ' Tambah Opsi</button></div>' +

      '<div class="field full" id="areaPembahasan">' +
      '<label class="label">Pembahasan <small class="muted">(opsional, hanya dilihat pengajar)</small></label>' +
      '<textarea class="textarea" name="Pembahasan" rows="2">' + esc(d.Pembahasan || '') + '</textarea></div>' +

      '<div class="field"><label class="label">Bobot Nilai</label>' +
      '<input class="input" type="number" name="Bobot" min="1" max="100" value="' + esc(d.Bobot || 1) + '"></div>' +

      '<div class="field"><label class="label">Tingkat Kesulitan</label><select class="select" name="Tingkat">' +
      opsiSelect(['Mudah', 'Sedang', 'Sulit'], null, null, d.Tingkat || 'Sedang') + '</select></div>' +

      '<div class="field"><label class="label">Tag <small class="muted">(opsional)</small></label>' +
      '<input class="input" name="Tag" value="' + esc(d.Tag || '') + '" placeholder="Bab 3, HOTS"></div>' +
      '</div></form>',
    foot: '<button class="btn btn-outline" onclick="closeModal()">Batal</button>' +
      '<button class="btn btn-primary" id="btnSimpanSoal">' + svgIcon('check', 18) + ' Simpan Soal</button>',
    onOpen: function () {
      $('#btnSimpanSoal').onclick = function () { simpanSoal(id); };
    }
  });
}

function barisOpsi(i, nilai, terpilih) {
  const huruf = String.fromCharCode(65 + i);
  return '<div class="opsi-row" data-idx="' + i + '">' +
    '<label class="opsi-radio" title="Tandai sebagai jawaban benar">' +
    '<input type="radio" name="kunciOpsi" value="' + i + '"' + (terpilih ? ' checked' : '') + '>' +
    '<span class="opsi-huruf">' + huruf + '</span></label>' +
    '<input class="input opsi-teks" value="' + esc(nilai || '') + '" placeholder="Teks pilihan ' + huruf + '">' +
    '<button type="button" class="btn btn-ghost btn-sm danger" title="Hapus opsi" onclick="hapusOpsi(this)">' +
    svgIcon('trash', 15) + '</button></div>';
}

function ubahJenisSoal() {
  const jenis = $('#jenisSoalInput').value;
  const area = $('#areaOpsiPG');
  if (area) area.hidden = (jenis !== 'PG');
}

function tambahOpsi() {
  const wrap = $('#daftarOpsi');
  const i = wrap.children.length;
  if (i >= 8) { showToast('Maksimal 8 pilihan jawaban.', 'warning'); return; }
  wrap.insertAdjacentHTML('beforeend', barisOpsi(i, '', false));
  renderIcons(wrap);
}

function hapusOpsi(btn) {
  const wrap = $('#daftarOpsi');
  if (wrap.children.length <= 2) { showToast('Pilihan ganda memerlukan minimal 2 opsi.', 'warning'); return; }
  btn.closest('.opsi-row').remove();
  /* Beri nomor ulang agar huruf A, B, C tetap berurutan setelah penghapusan. */
  Array.prototype.slice.call(wrap.children).forEach(function (row, i) {
    row.dataset.idx = i;
    row.querySelector('.opsi-huruf').textContent = String.fromCharCode(65 + i);
    const radio = row.querySelector('input[type=radio]');
    radio.value = i;
    row.querySelector('.opsi-teks').placeholder = 'Teks pilihan ' + String.fromCharCode(65 + i);
  });
}

function simpanSoal(id) {
  if (!validForm('formSoal')) return;
  const p = nilaiForm('formSoal');
  const payload = {
    ID: id || undefined,
    MapelID: p.MapelID, Jenis: p.Jenis, Pertanyaan: p.Pertanyaan,
    Pembahasan: p.Pembahasan, Bobot: Number(p.Bobot) || 1,
    Tingkat: p.Tingkat, Tag: p.Tag
  };

  if (p.Jenis === 'PG') {
    const teks = $$('#daftarOpsi .opsi-teks').map(function (i) { return i.value.trim(); });
    const kunci = $('#daftarOpsi input[name=kunciOpsi]:checked');
    if (teks.filter(Boolean).length < 2) {
      showToast('Isi minimal 2 pilihan jawaban.', 'warning'); return;
    }
    if (!kunci) { showToast('Tandai dulu pilihan mana yang merupakan jawaban benar.', 'warning'); return; }
    if (!teks[Number(kunci.value)]) {
      showToast('Pilihan yang Anda tandai sebagai kunci masih kosong.', 'warning'); return;
    }
    payload.Opsi = teks;
    payload.KunciJawaban = Number(kunci.value);
  }

  api('apiSimpanBankSoal', AppState.sessionToken, payload).then(function (res) {
    if (!res.success) { showToast(res.message, 'error'); return; }
    closeModal(); showToast(res.message, 'success'); muatBankSoal();
  }).catch(function (e) { showToast(e.message, 'error'); });
}

function hapusSoal(id) {
  konfirmasi('Hapus soal ini dari bank soal? Bila soal sedang dipakai ujian, ia akan diarsipkan alih-alih dihapus.',
    function () {
      api('apiHapusBankSoal', AppState.sessionToken, [id]).then(function (res) {
        showToast(res.message, res.success ? 'success' : 'error');
        if (res.success) muatBankSoal();
      }).catch(function (e) { showToast(e.message, 'error'); });
    }, { danger: true, labelYa: 'Ya, Hapus' });
}

/** Impor bank soal dari berkas Excel. */
function formImporSoal() {
  openModal({
    title: 'Impor Bank Soal dari Excel', size: 'wide',
    body: '<div class="alert alert-info"><i data-icon="info"></i><div>' +
      '<strong>Susunan kolom yang dikenali</strong>' +
      '<p>Baris pertama berkas harus berisi nama kolom persis seperti ini:</p></div></div>' +
      '<div class="table-wrap"><table class="tbl"><thead><tr>' +
      '<th>Jenis</th><th>Pertanyaan</th><th>OpsiA</th><th>OpsiB</th><th>OpsiC</th><th>OpsiD</th>' +
      '<th>Kunci</th><th>Bobot</th><th>Tingkat</th></tr></thead><tbody>' +
      '<tr><td>PG</td><td>Ibu kota Indonesia?</td><td>Jakarta</td><td>Bandung</td><td>Surabaya</td><td>Medan</td>' +
      '<td>A</td><td>1</td><td>Mudah</td></tr>' +
      '<tr><td>Uraian</td><td>Jelaskan proses fotosintesis.</td><td colspan="4" class="muted">(dikosongkan)</td>' +
      '<td class="muted">—</td><td>5</td><td>Sulit</td></tr>' +
      '</tbody></table></div>' +
      '<p class="hint" style="margin-top:10px">Kolom <b>Kunci</b> diisi huruf A–E, bukan angka. ' +
      'Untuk soal Uraian dan Unggah Berkas, kolom opsi dan kunci dibiarkan kosong.</p>' +
      '<div class="form-grid" style="margin-top:16px">' +
      '<div class="field"><label class="label">Mata Pelajaran <span class="req">*</span></label>' +
      '<select class="select" id="mapelImporSoal" required>' +
      opsiSelect(mapelDiampu(), 'ID', function (x) { return x.Kode + ' — ' + x.Nama; }, '', '— Pilih —') +
      '</select></div>' +
      '<div class="field"><label class="label">Berkas Excel / CSV <span class="req">*</span></label>' +
      '<input class="input" type="file" id="berkasImporSoal" accept=".xlsx,.xls,.csv"></div>' +
      '</div><div id="pratinjauImporSoal"></div>',
    foot: '<button class="btn btn-outline" onclick="closeModal()">Batal</button>' +
      '<button class="btn btn-outline" onclick="unduhTemplatSoal()">' + svgIcon('download', 18) + ' Unduh Templat</button>' +
      '<button class="btn btn-primary" id="btnProsesImporSoal">' + svgIcon('upload', 18) + ' Impor</button>',
    onOpen: function () {
      $('#btnProsesImporSoal').onclick = prosesImporSoal;
    }
  });
}

function unduhTemplatSoal() {
  if (typeof XLSX === 'undefined') { showToast('Pustaka Excel belum termuat. Muat ulang halaman.', 'error'); return; }
  const baris = [
    ['Jenis', 'Pertanyaan', 'OpsiA', 'OpsiB', 'OpsiC', 'OpsiD', 'OpsiE', 'Kunci', 'Bobot', 'Tingkat', 'Tag', 'Pembahasan'],
    ['PG', 'Ibu kota Indonesia adalah…', 'Jakarta', 'Bandung', 'Surabaya', 'Medan', '', 'A', 1, 'Mudah', 'Bab 1', ''],
    ['Uraian', 'Jelaskan proses fotosintesis secara ringkas.', '', '', '', '', '', '', 5, 'Sulit', 'Bab 3', ''],
    ['Upload', 'Unggah hasil laporan praktikum Anda dalam format PDF.', '', '', '', '', '', '', 10, 'Sedang', 'Praktikum', '']
  ];
  const ws = XLSX.utils.aoa_to_sheet(baris);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Bank Soal');
  XLSX.writeFile(wb, 'Templat_Bank_Soal.xlsx');
}

function prosesImporSoal() {
  const mapelId = $('#mapelImporSoal').value;
  const berkas  = $('#berkasImporSoal').files[0];
  if (!mapelId) { showToast('Pilih mata pelajaran terlebih dahulu.', 'warning'); return; }
  if (!berkas)  { showToast('Pilih berkas Excel atau CSV.', 'warning'); return; }
  if (typeof XLSX === 'undefined') { showToast('Pustaka Excel belum termuat. Muat ulang halaman.', 'error'); return; }
  ...
  reader.onload = function (ev) {
    ...
    try {
      ...
      showToast('Berkas tidak dapat dibaca: ' + err.message, 'error'); return;
    }
    if (!rows.length) { showToast('Berkas tidak berisi baris data.', 'warning'); return; }

    api('apiImporBankSoal', AppState.sessionToken, mapelId, rows).then(function (res) {
      const galat = (res.data && res.data.galat) || [];
      if (galat.length) {
        $('#pratinjauImporSoal').innerHTML =
          '<div class="alert alert-warn" style="margin-top:14px"><i data-icon="alert-triangle"></i><div>' +
          '<strong>' + galat.length + ' baris dilewati</strong><ul style="margin:6px 0 0;padding-left:18px">' +
          galat.slice(0, 10).map(function (g) { return '<li>' + esc(g) + '</li>'; }).join('') +
          (galat.length > 10 ? '<li>… dan ' + (galat.length - 10) + ' lainnya</li>' : '') +
          '</ul></div></div>';
        renderIcons($('#pratinjauImporSoal'));
      }
      showToast(res.message, res.success ? 'success' : 'error');
      if (res.success) { muatBankSoal(); if (!galat.length) closeModal(); }
    }).catch(function (e) { showToast(e.message, 'error'); });
  };
...
/* -------------------------------------------------------------------------- */
/* H2. DAFTAR & PENYUSUNAN UJIAN (PENGAJAR)                                   */
/* -------------------------------------------------------------------------- */

PAGES['dosen-ujian'] = function () {
  return headerHalaman('Ujian',
    'Susun ujian dari bank soal, pantau pengerjaan, dan tangani pelanggaran.',
    '<button class="btn btn-primary" onclick="formUjian()"><i data-icon="plus"></i><span>Buat Ujian</span></button>') +
    '<div id="areaDaftarUjian"><section class="card"><div class="card-body"><div class="skeleton-list">' +
    '<div class="skeleton-row"></div><div class="skeleton-row"></div></div></div></section></div>';
};

PAGE_INIT['dosen-ujian'] = function () { muatDaftarUjian(); };

function muatDaftarUjian() {
  api('apiGetUjianDosen', AppState.sessionToken, '').then(function (res) {
    const area = $('#areaDaftarUjian');
    if (!area) return;
    if (!res.success) {
      area.innerHTML = '<section class="card"><div class="card-body">' +
        kosongState('alert-circle', 'Gagal memuat ujian', res.message) + '</div></section>';
      return;
    }
    UJIAN_STATE.daftarUjian = res.data || [];
    gambarDaftarUjian();
  }).catch(function (e) {
    const area = $('#areaDaftarUjian');
    if (area) area.innerHTML = '<section class="card"><div class="card-body">' +
      kosongState('alert-circle', 'Gagal memuat', e.message) + '</div></section>';
  });
}

function gambarDaftarUjian() {
  const area = $('#areaDaftarUjian');
  if (!area) return;
  const data = UJIAN_STATE.daftarUjian;

  if (!data.length) {
    area.innerHTML = '<section class="card"><div class="card-body">' +
      kosongState('notebook-pen', 'Belum ada ujian',
        'Susun bank soal terlebih dahulu, lalu buat ujian dari soal-soal tersebut.',
        '<button class="btn btn-primary" onclick="formUjian()">' + svgIcon('plus', 18) + ' Buat Ujian Pertama</button>') +
      '</div></section>';
    return;
  }

  const perluPerhatian = data.reduce(function (t, u) { return t + (u.statistik.diblokir || 0); }, 0);
  const belumDinilai   = data.reduce(function (t, u) { return t + (u.statistik.belumDinilai || 0); }, 0);

  area.innerHTML =
    (perluPerhatian || belumDinilai
      ? '<div class="alert ' + (perluPerhatian ? 'alert-warn' : 'alert-info') + '">' +
        '<i data-icon="' + (perluPerhatian ? 'ban' : 'clipboard-check') + '"></i><div>' +
        '<strong>' + (perluPerhatian ? perluPerhatian + ' siswa terblokir' : belumDinilai + ' pekerjaan menunggu koreksi') + '</strong>' +
        '<p>' + (perluPerhatian
          ? 'Siswa yang terblokir tidak dapat melanjutkan ujian sampai Anda membuka aksesnya.'
          : 'Soal uraian dan unggahan berkas perlu dinilai manual agar nilai akhir terbit.') + '</p>' +
        '</div></div>'
      : '') +
    '<div class="grid-cards">' + data.map(kartuUjian).join('') + '</div>';
  renderIcons(area);
}

function kartuUjian(u) {
  const st = u.statistik || {};
  const statusBadge = {
    'Draf':   '<span class="badge badge-neutral">Draf</span>',
    'Terbit': '<span class="badge badge-success">Terbit</span>',
    'Arsip':  '<span class="badge badge-neutral plain">Arsip</span>'
  }[u.Status] || '<span class="badge badge-neutral">' + esc(u.Status) + '</span>';

  const jendela = u.TanggalMulai
    ? fmtTgl(u.TanggalMulai, true) + ' – ' + (u.TanggalSelesai ? fmtTgl(u.TanggalSelesai, true) : 'tanpa batas')
    : 'Jadwal belum ditentukan';

  return '<article class="card ujian-card">' +
    '<div class="ujian-card-head">' +
      '<div><h3>' + esc(u.Judul) + '</h3>' +
      '<p class="muted">' + esc(namaMapel(u.MapelID)) + ' · ' + esc(namaKelas(u.KelasID)) + '</p></div>' +
      statusBadge +
    '</div>' +
    '<div class="ujian-meta">' +
      '<span>' + svgIcon('list-checks', 15) + ' ' + u.jumlahSoal + ' soal</span>' +
      '<span>' + svgIcon('timer', 15) + ' ' + u.DurasiMenit + ' menit</span>' +
      '<span>' + svgIcon('star', 15) + ' bobot ' + u.TotalBobot + '</span>' +
      (u.AcakSoal ? '<span title="Urutan soal diacak per siswa">' + svgIcon('shuffle', 15) + ' acak</span>' : '') +
    '</div>' +
    '<p class="ujian-jendela">' + svgIcon('clock', 14) + ' ' + esc(jendela) + '</p>' +
    '<div class="ujian-stat">' +
      '<div><b>' + (st.total || 0) + '</b><span>ikut</span></div>' +
      '<div><b>' + (st.selesai || 0) + '</b><span>selesai</span></div>' +
      '<div class="' + (st.berlangsung ? 'hi' : '') + '"><b>' + (st.berlangsung || 0) + '</b><span>berjalan</span></div>' +
      '<div class="' + (st.diblokir ? 'danger' : '') + '"><b>' + (st.diblokir || 0) + '</b><span>terblokir</span></div>' +
    '</div>' +
    '<div class="ujian-card-foot">' +
      '<button class="btn btn-outline btn-sm" onclick="formUjian(\'' + esc(u.ID) + '\')">' + svgIcon('edit', 15) + ' Ubah</button>' +
      (u.Status === 'Draf'
        ? '<button class="btn btn-primary btn-sm" onclick="ubahStatusUjian(\'' + esc(u.ID) + '\',\'Terbit\')">' + svgIcon('send', 15) + ' Terbitkan</button>'
        : '<button class="btn btn-outline btn-sm" onclick="ubahStatusUjian(\'' + esc(u.ID) + '\',\'Draf\')">' + svgIcon('lock', 15) + ' Tarik</button>') +
      '<button class="btn btn-primary btn-sm" onclick="navigateTo(\'dosen-ujian-hasil\',{ctx:{ujianId:\'' + esc(u.ID) + '\'}})">' +
        svgIcon('bar-chart', 15) + ' Hasil' +
        (st.belumDinilai ? ' <span class="dot-badge">' + st.belumDinilai + '</span>' : '') +
      '</button>' +
      '<button class="btn btn-ghost btn-sm danger" title="Hapus" onclick="hapusUjian(\'' + esc(u.ID) + '\')">' + svgIcon('trash', 15) + '</button>' +
    '</div></article>';
}

function ubahStatusUjian(id, status) {
  const u = UJIAN_STATE.daftarUjian.filter(function (x) { return x.ID === id; })[0];
  if (!u) return;

  const lanjut = function () {
    api('apiGetUjianDosen', AppState.sessionToken, id).then(function (detail) {
      if (!detail.success) { showToast(detail.message, 'error'); return; }
      const p = Object.assign({}, detail.data.ujian, { Status: status });
      p.soal = detail.data.soal.map(function (s) {
        return { SoalID: s.SoalID, Urutan: s.Urutan, Bobot: s.Bobot };
      });
      api('apiSimpanUjian', AppState.sessionToken, p).then(function (res) {
        showToast(status === 'Terbit' ? 'Ujian diterbitkan — siswa sudah dapat melihatnya.' : 'Ujian ditarik kembali menjadi draf.',
                  res.success ? 'success' : 'error');
        if (res.success) muatDaftarUjian();
      }).catch(function (e) { showToast(e.message, 'error'); });
    }).catch(function (e) { showToast(e.message, 'error'); });
  };

  if (status === 'Terbit') {
    konfirmasi('Terbitkan "' + u.Judul + '"? Siswa di kelas ' + namaKelas(u.KelasID) +
               ' akan langsung melihat ujian ini sesuai jadwal yang Anda tetapkan.',
               lanjut, { labelYa: 'Ya, Terbitkan' });
  } else {
    konfirmasi('Tarik "' + u.Judul + '" kembali menjadi draf? Siswa tidak akan bisa memulai ujian ini. ' +
               'Siswa yang sudah mengerjakan tetap tersimpan hasilnya.',
               lanjut, { labelYa: 'Ya, Tarik' });
  }
}

function hapusUjian(id) {
  konfirmasi('Hapus ujian ini? Bila sudah ada siswa yang mengerjakan, ujian akan diarsipkan alih-alih dihapus agar nilai mereka tidak hilang.',
    function () {
      api('apiHapusUjian', AppState.sessionToken, id).then(function (res) {
        showToast(res.message, res.success ? 'success' : 'error');
        if (res.success) muatDaftarUjian();
      }).catch(function (e) { showToast(e.message, 'error'); });
    }, { danger: true, labelYa: 'Ya, Hapus' });
...
/** Formulir buat/ubah ujian — dua langkah: pengaturan, lalu pemilihan soal. */
function formUjian(id) {
  if (!id) { tampilFormUjian({}, []); return; }
  api('apiGetUjianDosen', AppState.sessionToken, id).then(function (res) {
    if (!res.success) { showToast(res.message, 'error'); return; }
    tampilFormUjian(res.data.ujian, res.data.soal);
  }).catch(function (e) { showToast(e.message, 'error'); });
}

function tampilFormUjian(d, soalTerpilih) {
  const nilaiDatetime = function (iso) {
    if (!iso) return '';
  ...
  /* Soal terpilih disimpan di state agar bertahan saat pengguna bolak-balik
     antara tab Pengaturan dan tab Soal di dalam modal yang sama. */
  UJIAN_STATE.soalTerpilih = (soalTerpilih || []).map(function (s) {
    return { SoalID: s.SoalID, Urutan: s.Urutan, Bobot: s.Bobot,
             Jenis: s.Jenis, Pertanyaan: s.Pertanyaan };
  });

  openModal({
    title: d.ID ? 'Ubah Ujian' : 'Buat Ujian Baru', size: 'wide',
    body:
      '<div class="tabs" role="tablist">' +
        '<button class="tab active" onclick="tabUjian(0,this)">1. Pengaturan</button>' +
        '<button class="tab" onclick="tabUjian(1,this)">2. Pilih Soal <span id="jmlSoalTab" class="tab-count">' +
          UJIAN_STATE.soalTerpilih.length + '</span></button>' +
      '</div>' +

      '<div id="tabUjian0"><form id="formUjian" onsubmit="return false"><div class="form-grid">' +
        '<div class="field full"><label class="label">Judul Ujian <span class="req">*</span></label>' +
        '<input class="input" name="Judul" value="' + esc(d.Judul || '') + '" required ' +
        'placeholder="Ujian Tengah Semester — Kalkulus Lanjut"></div>' +

        '<div class="field full"><label class="label">Petunjuk untuk Siswa</label>' +
        '<textarea class="textarea" name="Deskripsi" rows="2" ' +
        'placeholder="Kerjakan sendiri. Dilarang membuka tab lain selama ujian berlangsung.">' + esc(d.Deskripsi || '') + '</textarea></div>' +
        ...
        '<div class="field"><label class="label">Kelas <span class="req">*</span></label>' +
        '<select class="select" name="KelasID" required>' +
        opsiSelect(kelasDiampu(), 'ID', 'Nama', d.KelasID, '— Pilih —') + '</select></div>' +

        '<div class="field"><label class="label">Jenis Ujian</label><select class="select" name="Jenis">' +
        opsiSelect(['Quiz', 'UTS', 'UAS', 'Ujian Praktik', 'Remedial'], null, null, d.Jenis || 'Quiz') + '</select></div>' +

        '<div class="field"><label class="label">Durasi (menit) <span class="req">*</span></label>' +
        '<input class="input" type="number" name="DurasiMenit" min="1" max="360" required ' +
        'value="' + esc(d.DurasiMenit || 60) + '"></div>' +

        '<div class="field"><label class="label">Waktu Buka</label>' +
        '<input class="input" type="datetime-local" name="TanggalMulai" value="' + esc(nilaiDatetime(d.TanggalMulai)) + '"></div>' +

        '<div class="field"><label class="label">Waktu Tutup</label>' +
        '<input class="input" type="datetime-local" name="TanggalSelesai" value="' + esc(nilaiDatetime(d.TanggalSelesai)) + '"></div>' +

        '<div class="field full"><label class="label">Pengawasan & Pengacakan</label>' +
        '<div class="switch-list">' +
          switchBaris('AcakSoal', 'Acak urutan soal', 'Tiap siswa menerima urutan soal yang berbeda.', d.AcakSoal !== false) +
          switchBaris('AcakOpsi', 'Acak urutan pilihan jawaban', 'Berlaku untuk soal pilihan ganda.', d.AcakOpsi !== false) +
          switchBaris('TampilkanNilai', 'Tampilkan nilai ke siswa setelah selesai', 'Nilai baru muncul bila seluruh butir sudah dinilai.', !!d.TampilkanNilai) +
        '</div></div>' +

        '<div class="field"><label class="label">Batas Pelanggaran</label>' +
        '<select class="select" name="MaksPelanggaran">' +
        '<option value="2"' + (Number(d.MaksPelanggaran || 2) === 2 ? ' selected' : '') + '>2 — peringatan sekali, lalu blokir</option>' +
        '<option value="3"' + (Number(d.MaksPelanggaran) === 3 ? ' selected' : '') + '>3 — peringatan dua kali, lalu blokir</option>' +
        '<option value="1"' + (Number(d.MaksPelanggaran) === 1 ? ' selected' : '') + '>1 — langsung blokir tanpa peringatan</option>' +
        '</select>' +
        '<p class="hint">Siswa yang meninggalkan halaman ujian (pindah tab atau memperkecil jendela) dihitung melanggar.</p></div>' +
      '</div></form></div>' +

      '<div id="tabUjian1" hidden>' +
        '<div class="row" style="margin-bottom:12px;gap:8px">' +
        '<input class="input" id="cariSoalUjian" placeholder="Cari soal…" oninput="gambarPemilihSoal()" style="flex:1">' +
        '<select class="select" id="filterJenisSoalUjian" onchange="gambarPemilihSoal()" style="width:170px">' +
        '<option value="">Semua jenis</option><option value="PG">Pilihan Ganda</option>' +
        '<option value="Uraian">Uraian</option><option value="Upload">Unggah Berkas</option></select>' +
        '</div>' +
        '<div id="areaPemilihSoal"><p class="muted">Memuat bank soal…</p></div>' +
      '</div>',
    foot: '<button class="btn btn-outline" onclick="closeModal()">Batal</button>' +
      '<span class="foot-note" id="ringkasUjian"></span>' +
      '<button class="btn btn-primary" id="btnSimpanUjian">' + svgIcon('check', 18) + ' Simpan Ujian</button>',
    onOpen: function () {
      perbaruiRingkasUjian();
      $('#btnSimpanUjian').onclick = function () { simpanUjian(d.ID); };
      /* Muat bank soal untuk mapel yang dipilih, dan muat ulang bila mapel diganti. */
      const selMapel = $('#formUjian [name=MapelID]');
      const muat = function () {
        api('apiGetBankSoal', AppState.sessionToken, selMapel.value || '').then(function (res) {
          UJIAN_STATE.bankSoal = res.success ? (res.data || []) : [];
          gambarPemilihSoal();
        }).catch(function () { UJIAN_STATE.bankSoal = []; gambarPemilihSoal(); });
      };
      selMapel.addEventListener('change', muat);
      muat();
    }
  });
}

function switchBaris(nama, judul, keterangan, aktif) {
  return '<label class="switch-row">' +
    '<input type="checkbox" name="' + nama + '"' + (aktif ? ' checked' : '') + '>' +
    '<span class="switch-track"><span class="switch-knob"></span></span>' +
    '<span class="switch-text"><b>' + esc(judul) + '</b><small>' + esc(keterangan) + '</small></span></label>';
}

function tabUjian(idx, btn) {
  $$('.tabs .tab').forEach(function (t) { t.classList.remove('active'); });
  btn.classList.add('active');
  $('#tabUjian0').hidden = (idx !== 0);
  $('#tabUjian1').hidden = (idx !== 1);
}

function gambarPemilihSoal() {
  const area = $('#areaPemilihSoal');
  if (!area) return;

  const cari  = ($('#cariSoalUjian') || {}).value || '';
  const jenis = ($('#filterJenisSoalUjian') || {}).value || '';
  const terpilihIds = UJIAN_STATE.soalTerpilih.map(function (s) { return String(s.SoalID); });

  const tersedia = UJIAN_STATE.bankSoal.filter(function (s) {
    if (jenis && s.Jenis !== jenis) return false;
    if (cari && String(s.Pertanyaan).toLowerCase().indexOf(cari.toLowerCase()) === -1) return false;
    return true;
  });

  if (!UJIAN_STATE.bankSoal.length) {
    area.innerHTML = kosongState('library', 'Bank soal kosong untuk mata pelajaran ini',
      'Tambahkan soal lebih dulu di menu Bank Soal, lalu kembali ke sini untuk menyusun ujian.');
    return;
  }

  area.innerHTML =
    '<div class="pilih-soal-grid">' +
      '<div class="pilih-kolom"><h4>Tersedia <small>(' + tersedia.length + ')</small></h4>' +
      '<div class="pilih-list">' +
      (tersedia.length ? tersedia.map(function (s) {
        const sudah = terpilihIds.indexOf(String(s.ID)) !== -1;
        return '<div class="pilih-item' + (sudah ? ' dipakai' : '') + '">' +
          '<div class="pilih-item-body">' + badgeJenisSoal(s.Jenis) +
          '<p>' + esc(String(s.Pertanyaan).slice(0, 110)) + (String(s.Pertanyaan).length > 110 ? '…' : '') + '</p>' +
          '<small class="muted">Bobot ' + esc(s.Bobot) + ' · ' + esc(s.Tingkat || 'Sedang') + '</small></div>' +
          (sudah
            ? '<span class="badge badge-success plain">Dipakai</span>'
            : '<button class="btn btn-outline btn-sm" onclick="tambahSoalKeUjian(\'' + esc(s.ID) + '\')">' +
              svgIcon('plus', 15) + '</button>') +
          '</div>';
      }).join('') : '<p class="muted" style="padding:12px">Tidak ada soal yang cocok dengan pencarian.</p>') +
      '</div></div>' +

      '<div class="pilih-kolom"><h4>Dipakai di Ujian <small>(' + UJIAN_STATE.soalTerpilih.length + ')</small></h4>' +
      '<div class="pilih-list">' +
      (UJIAN_STATE.soalTerpilih.length ? UJIAN_STATE.soalTerpilih.map(function (s, i) {
        return '<div class="pilih-item terpilih">' +
          '<span class="pilih-nomor">' + (i + 1) + '</span>' +
          '<div class="pilih-item-body">' + badgeJenisSoal(s.Jenis) +
          '<p>' + esc(String(s.Pertanyaan || '').slice(0, 100)) + '</p></div>' +
          '<input class="input bobot-soal" type="number" min="1" max="100" value="' + esc(s.Bobot) + '" ' +
          'data-id="' + esc(s.SoalID) + '" onchange="ubahBobotSoal(this)" title="Bobot nilai soal ini">' +
          '<button class="btn btn-ghost btn-sm danger" onclick="buangSoalDariUjian(\'' + esc(s.SoalID) + '\')">' +
          svgIcon('trash', 15) + '</button></div>';
      }).join('') : '<p class="muted" style="padding:12px">Belum ada soal dipilih. Tambahkan dari kolom kiri.</p>') +
      '</div></div>' +
    '</div>';

  renderIcons(area);
  const tabCount = $('#jmlSoalTab');
  if (tabCount) tabCount.textContent = UJIAN_STATE.soalTerpilih.length;
  perbaruiRingkasUjian();
}

function tambahSoalKeUjian(soalId) {
  const s = UJIAN_STATE.bankSoal.filter(function (x) { return x.ID === soalId; })[0];
  if (!s) return;
  if (UJIAN_STATE.soalTerpilih.some(function (x) { return String(x.SoalID) === String(soalId); })) return;
  UJIAN_STATE.soalTerpilih.push({
    SoalID: s.ID, Urutan: UJIAN_STATE.soalTerpilih.length + 1,
    Bobot: Number(s.Bobot) || 1, Jenis: s.Jenis, Pertanyaan: s.Pertanyaan
  });
  gambarPemilihSoal();
}

function buangSoalDariUjian(soalId) {
  UJIAN_STATE.soalTerpilih = UJIAN_STATE.soalTerpilih
    .filter(function (x) { return String(x.SoalID) !== String(soalId); })
    .map(function (x, i) { x.Urutan = i + 1; return x; });
  gambarPemilihSoal();
}

function ubahBobotSoal(input) {
  const s = UJIAN_STATE.soalTerpilih.filter(function (x) { return String(x.SoalID) === input.dataset.id; })[0];
  if (s) s.Bobot = Math.max(1, Number(input.value) || 1);
  perbaruiRingkasUjian();
}

function perbaruiRingkasUjian() {
  const el = $('#ringkasUjian');
  if (!el) return;
  const n = UJIAN_STATE.soalTerpilih.length;
  const bobot = UJIAN_STATE.soalTerpilih.reduce(function (t, s) { return t + (Number(s.Bobot) || 1); }, 0);
  el.innerHTML = n
    ? n + ' soal · total bobot <b>' + bobot + '</b>'
    : '<span class="muted">Belum ada soal dipilih</span>';
}

function simpanUjian(id) {
  if (!validForm('formUjian')) { tabUjian(0, $$('.tabs .tab')[0]); return; }
  if (!UJIAN_STATE.soalTerpilih.length) {
    showToast('Pilih minimal satu soal di tab "Pilih Soal".', 'warning');
    tabUjian(1, $$('.tabs .tab')[1]);
    return;
  }

  const p = nilaiForm('formUjian');
  const payload = {
    ID: id || undefined,
    Judul: p.Judul, Deskripsi: p.Deskripsi,
    MapelID: p.MapelID, KelasID: p.KelasID, Jenis: p.Jenis,
    DurasiMenit: Number(p.DurasiMenit) || 60,
    TanggalMulai:   p.TanggalMulai   || '',
    TanggalSelesai: p.TanggalSelesai || '',
    AcakSoal:       p.AcakSoal === 'TRUE',
    AcakOpsi:       p.AcakOpsi === 'TRUE',
    TampilkanNilai: p.TampilkanNilai === 'TRUE',
    MaksPelanggaran: Number(p.MaksPelanggaran) || 2,
    Semester:    (DB.institusi || {}).SemesterAktif || '',
    TahunAjaran: (DB.institusi || {}).TahunAjaran   || '',
    Status: 'Draf',
    soal: UJIAN_STATE.soalTerpilih.map(function (s, i) {
      return { SoalID: s.SoalID, Urutan: i + 1, Bobot: Number(s.Bobot) || 1 };
    })
  };
  /* Status yang sudah terbit dipertahankan agar menyimpan suntingan kecil
     tidak diam-diam menarik ujian yang sedang berjalan. */
  const lama = UJIAN_STATE.daftarUjian.filter(function (x) { return x.ID === id; })[0];
  if (lama) payload.Status = lama.Status;

  api('apiSimpanUjian', AppState.sessionToken, payload).then(function (res) {
    if (!res.success) { showToast(res.message, 'error'); return; }
    closeModal(); showToast(res.message, 'success'); muatDaftarUjian();
  }).catch(function (e) { showToast(e.message, 'error'); });
}


/* -------------------------------------------------------------------------- */
/* H3. HASIL, KOREKSI & PENGAWASAN                                            */
/* -------------------------------------------------------------------------- */

PAGES['dosen-ujian-hasil'] = function (ctx) {
  const id = (ctx || {}).ujianId;
  if (!id) return kosongState('alert-circle', 'Ujian tidak dipilih', 'Kembali ke daftar ujian lalu pilih salah satu.');
  return '<div id="areaHasilUjian"><div class="card"><div class="card-body">' +
    '<div class="skeleton-list"><div class="skeleton-row"></div><div class="skeleton-row"></div></div>' +
    '</div></div></div>';
};

PAGE_INIT['dosen-ujian-hasil'] = function (ctx) {
  muatHasilUjian((ctx || {}).ujianId);
};

function muatHasilUjian(ujianId) {
  api('apiGetHasilUjian', AppState.sessionToken, ujianId).then(function (res) {
    const area = $('#areaHasilUjian');
    if (!area) return;
    if (!res.success) {
      area.innerHTML = kosongState('alert-circle', 'Gagal memuat hasil', res.message);
      return;
    }
    UJIAN_STATE.hasil = res.data;
    gambarHasilUjian();
  }).catch(function (e) {
    const area = $('#areaHasilUjian');
    if (area) area.innerHTML = kosongState('alert-circle', 'Gagal memuat', e.message);
  });
}

function gambarHasilUjian() {
  const area = $('#areaHasilUjian');
  if (!area) return;
  const h = UJIAN_STATE.hasil;
  const u = h.ujian;

  const terblokir   = h.sesi.filter(function (s) { return s.status === 'Diblokir'; });
  const berlangsung = h.sesi.filter(function (s) { return s.status === 'Berlangsung'; });
  const selesai     = h.sesi.filter(function (s) { return s.status === 'Selesai'; });
  const perluNilai  = selesai.filter(function (s) { return !s.dinilaiAt; });

  const nilaiTerbit = selesai.filter(function (s) { return s.nilaiTotal !== null; });
  const rata = nilaiTerbit.length
    ? (nilaiTerbit.reduce(function (t, s) { return t + s.nilaiTotal; }, 0) / nilaiTerbit.length).toFixed(1)
    : '—';

  area.innerHTML =
    headerHalaman(u.Judul, namaMapel(u.MapelID) + ' · ' + namaKelas(u.KelasID) +
      ' · ' + u.DurasiMenit + ' menit · total bobot ' + u.TotalBobot,
      '<button class="btn btn-outline" onclick="goBack()"><i data-icon="corner-up-left"></i><span>Kembali</span></button>' +
      '<button class="btn btn-outline" onclick="unduhHasilUjian()"><i data-icon="download"></i><span>Unduh Excel</span></button>') +

    '<div class="kpi-grid">' +
      kpiCard({ ikon: 'users', label: 'Mengerjakan', nilai: h.sesi.length, satuan: 'dari ' + (h.sesi.length + h.belumIkut.length) }) +
      kpiCard({ ikon: 'check-circle', label: 'Selesai', nilai: selesai.length, tint: 'var(--success-soft)', ink: 'var(--success)' }) +
      kpiCard({ ikon: 'ban', label: 'Terblokir', nilai: terblokir.length,
                tint: terblokir.length ? 'var(--error-soft)' : 'var(--sc)',
                ink: terblokir.length ? 'var(--error)' : 'var(--primary)' }) +
      kpiCard({ ikon: 'star', label: 'Rata-rata Nilai', nilai: rata, tint: 'var(--accent-soft)', ink: 'var(--accent-ink)' }) +
    '</div>' +

    (terblokir.length
      ? '<section class="card"><div class="card-head"><i data-icon="ban"></i>' +
        '<h2>Siswa Terblokir — Perlu Tindakan</h2></div><div class="card-body">' +
        '<p class="muted" style="margin-top:0">Siswa berikut meninggalkan halaman ujian melebihi batas. ' +
        'Jawaban yang sempat tersimpan tetap tercatat. Membuka akses akan memberi tambahan waktu dan menyetel ulang hitungan pelanggaran.</p>' +
        terblokir.map(kartuSiswaTerblokir).join('') + '</div></section>'
      : '') +

    (berlangsung.length
      ? '<section class="card"><div class="card-head"><i data-icon="timer"></i><h2>Sedang Mengerjakan</h2>' +
        '<div class="head-tools"><button class="btn btn-outline btn-sm" onclick="muatHasilUjian(\'' + esc(u.ID) + '\')">' +
        svgIcon('refresh', 15) + ' Segarkan</button></div></div>' +
        '<div class="card-body"><div class="table-wrap"><table class="tbl"><thead><tr>' +
        '<th>NIM</th><th>Nama</th><th>Mulai</th><th>Pelanggaran</th></tr></thead><tbody>' +
        berlangsung.map(function (s) {
          return '<tr><td>' + esc(s.nim) + '</td><td>' + esc(s.nama) + '</td>' +
            '<td>' + esc(fmtTgl(s.mulaiAt, true)) + '</td>' +
            '<td>' + (s.pelanggaran
              ? '<span class="badge badge-warn">' + s.pelanggaran + '×</span>'
              : '<span class="muted">—</span>') + '</td></tr>';
        }).join('') + '</tbody></table></div></div></section>'
      : '') +

    '<section class="card"><div class="card-head"><i data-icon="clipboard-check"></i><h2>Hasil & Koreksi</h2>' +
      (perluNilai.length ? '<span class="badge badge-warn">' + perluNilai.length + ' menunggu koreksi</span>' : '') +
    '</div><div class="card-body">' +
    (h.sesi.length
      ? '<div class="table-wrap"><table class="tbl"><thead><tr>' +
        '<th>NIM</th><th>Nama</th><th>Status</th><th class="num">Objektif</th>' +
        '<th class="num">Uraian</th><th class="num">Total</th><th>Pelanggaran</th><th></th>' +
        '</tr></thead><tbody>' + h.sesi.map(barisHasilSiswa).join('') + '</tbody></table></div>'
      : kosongState('inbox', 'Belum ada yang mengerjakan', 'Hasil akan muncul di sini begitu siswa mulai mengerjakan.')) +
    '</div></section>' +

    (h.belumIkut.length
      ? '<section class="card"><div class="card-head"><i data-icon="alert-circle"></i>' +
        '<h2>Belum Mengerjakan <small class="muted">(' + h.belumIkut.length + ')</small></h2></div>' +
        '<div class="card-body"><div class="chip-list">' +
        h.belumIkut.map(function (s) {
          return '<span class="chip">' + esc(s.nama) + ' <small>' + esc(s.nim) + '</small></span>';
...
function kartuSiswaTerblokir(s) {
  const log = (s.logPelanggaran || []).map(function (l) {
    return '<li>' + esc(l.jenis) + ' — ' + esc(fmtTgl(l.waktu, true)) + '</li>';
  }).join('');
  return '<div class="blokir-card">' +
    '<div class="blokir-info">' +
      '<div class="person"><span class="avatar">' + esc(inisial(s.nama)) + '</span>' +
      '<div><b>' + esc(s.nama) + '</b><br><small class="muted">' + esc(s.nim) + '</small></div></div>' +
      '<div class="blokir-detail">' +
        '<span class="badge badge-error">' + s.pelanggaran + '× meninggalkan halaman</span>' +
        (s.nilaiObjektif !== null ? '<span class="badge badge-neutral plain">Nilai sementara: ' + s.nilaiObjektif + '</span>' : '') +
      '</div>' +
      (log ? '<details class="blokir-log"><summary>Rincian pelanggaran</summary><ul>' + log + '</ul></details>' : '') +
    '</div>' +
    '<button class="btn btn-primary btn-sm" onclick="formBukaBlokir(\'' + esc(s.sesiID) + '\',\'' + esc(s.nama) + '\')">' +
    svgIcon('unlock', 16) + ' Buka Akses</button></div>';
}

function formBukaBlokir(sesiId, nama) {
  openModal({
    title: 'Buka Akses Ujian', size: 'slim',
    body: '<p>Membuka kembali akses ujian untuk <b>' + esc(nama) + '</b>.</p>' +
      '<div class="alert alert-info"><i data-icon="info"></i><div>' +
      '<p style="margin:0">Hitungan pelanggaran disetel ulang ke nol, dan siswa mendapat tambahan waktu agar tidak dirugikan dua kali. ' +
      'Bila ia kembali meninggalkan halaman, pemblokiran akan terjadi lagi.</p></div></div>' +
      '<form id="formBuka" onsubmit="return false"><div class="form-grid">' +
      '<div class="field"><label class="label">Tambahan Waktu (menit)</label>' +
      '<input class="input" type="number" name="tambahMenit" min="1" max="180" value="15"></div>' +
      '<div class="field full"><label class="label">Catatan <small class="muted">(opsional, tersimpan di riwayat)</small></label>' +
      '<input class="input" name="catatan" placeholder="Koneksi terputus, sudah diklarifikasi"></div>' +
      '</div></form>',
    foot: '<button class="btn btn-outline" onclick="closeModal()">Batal</button>' +
      '<button class="btn btn-primary" id="btnBukaBlokir">' + svgIcon('unlock', 18) + ' Buka Akses</button>',
    onOpen: function () {
      $('#btnBukaBlokir').onclick = function () {
        const p = nilaiForm('formBuka');
        api('apiBukaBlokirUjian', AppState.sessionToken, sesiId, p.catatan, Number(p.tambahMenit) || 15)
          .then(function (res) {
            showToast(res.message, res.success ? 'success' : 'error');
            if (res.success) { closeModal(); muatHasilUjian(UJIAN_STATE.hasil.ujian.ID); }
          }).catch(function (e) { showToast(e.message, 'error'); });
      };
    }
  });
...
function barisHasilSiswa(s) {
  const statusBadge = {
    'Selesai':     '<span class="badge badge-success">Selesai</span>',
    'Berlangsung': '<span class="badge badge-info">Berjalan</span>',
    'Diblokir':    '<span class="badge badge-error">Diblokir</span>'
  }[s.status] || '<span class="badge badge-neutral">' + esc(s.status) + '</span>';

  const perluKoreksi = s.status === 'Selesai' && !s.dinilaiAt;

  return '<tr>' +
    '<td>' + esc(s.nim) + '</td>' +
    '<td><div class="person"><span class="avatar">' + esc(inisial(s.nama)) + '</span>' + esc(s.nama) + '</div></td>' +
    '<td>' + statusBadge + (perluKoreksi ? ' <span class="badge badge-warn plain">perlu koreksi</span>' : '') + '</td>' +
    '<td class="num">' + (s.nilaiObjektif === null ? '<span class="muted">—</span>' : s.nilaiObjektif) + '</td>' +
    '<td class="num">' + (s.nilaiSubjektif === null ? '<span class="muted">—</span>' : s.nilaiSubjektif) + '</td>' +
    '<td class="num"><b>' + (s.nilaiTotal === null ? '<span class="muted">—</span>' : s.nilaiTotal) + '</b></td>' +
    '<td>' + (s.pelanggaran ? '<span class="badge badge-warn plain">' + s.pelanggaran + '×</span>' : '<span class="muted">—</span>') + '</td>' +
    '<td><button class="btn btn-outline btn-sm" onclick="bukaKoreksi(\'' + esc(s.sesiID) + '\')">' +
      svgIcon('eye', 15) + ' ' + (perluKoreksi ? 'Koreksi' : 'Lihat') + '</button></td>' +
    '</tr>';
}

/** Panel koreksi per siswa: jawaban, kunci, dan kolom nilai untuk butir manual. */
function bukaKoreksi(sesiId) {
  const s = UJIAN_STATE.hasil.sesi.filter(function (x) { return x.sesiID === sesiId; })[0];
  if (!s) return;

  const butir = s.butir.map(function (b) {
    const kotakNilai = b.perluDinilaiManual
      ? '<div class="koreksi-nilai">' +
        '<label class="label">Skor <small class="muted">(maks ' + b.Bobot + ')</small></label>' +
        '<input class="input skor-input" type="number" min="0" max="' + b.Bobot + '" step="0.5" ' +
        'data-soal="' + esc(b.SoalID) + '" value="' + (b.skor === null ? '' : esc(b.skor)) + '">' +
        '<input class="input fb-ujian-input" data-soal="' + esc(b.SoalID) + '" ' +
        'placeholder="Umpan balik (opsional)" value="' + esc(b.feedback || '') + '">' +
        '</div>'
      : '<div class="koreksi-nilai otomatis">' +
        (b.benar ? '<span class="badge badge-success">Benar · ' + b.skor + '</span>'
                 : '<span class="badge badge-error">Salah · 0</span>') +
        '</div>';

    const jawabanBlok = b.fileID
      ? '<button class="btn btn-outline btn-sm" onclick="previewFile(\'' +
        'https://drive.google.com/file/d/' + esc(b.fileID) + '/preview\',\'' + esc(b.fileNama || 'Berkas Jawaban') + '\',\'' +
        'https://drive.google.com/uc?export=download&id=' + esc(b.fileID) + '\')">' +
        svgIcon('file-text', 15) + ' ' + esc(b.fileNama || 'Lihat Berkas') + '</button>'
      : (b.jawabanTeks
          ? '<div class="jawaban-teks">' + esc(b.jawabanTeks) + '</div>'
          : '<span class="muted">Tidak dijawab</span>');

    return '<div class="koreksi-butir">' +
      '<div class="koreksi-head"><span class="koreksi-nomor">' + b.Nomor + '</span>' +
      badgeJenisSoal(b.Jenis) + '<span class="muted">bobot ' + b.Bobot + '</span></div>' +
      '<p class="koreksi-soal">' + esc(b.Pertanyaan) + '</p>' +
      '<div class="koreksi-jawab"><label class="label">Jawaban siswa</label>' + jawabanBlok + '</div>' +
      (b.kunciTeks ? '<div class="koreksi-kunci"><label class="label">Kunci</label>' +
        '<span>' + esc(b.kunciTeks) + '</span></div>' : '') +
      kotakNilai + '</div>';
  }).join('');

  const adaManual = s.butir.some(function (b) { return b.perluDinilaiManual; });

  openModal({
    title: 'Koreksi — ' + s.nama, size: 'wide',
    body: '<div class="row" style="margin-bottom:14px">' +
      '<span class="badge badge-neutral plain">NIM: <b>' + esc(s.nim) + '</b></span>' +
      '<span class="badge badge-neutral plain">Mulai: <b>' + esc(fmtTgl(s.mulaiAt, true)) + '</b></span>' +
      (s.selesaiAt ? '<span class="badge badge-neutral plain">Kumpul: <b>' + esc(fmtTgl(s.selesaiAt, true)) + '</b></span>' : '') +
      (s.pelanggaran ? '<span class="badge badge-warn">Pelanggaran: <b>' + s.pelanggaran + '×</b></span>' : '') +
      '</div>' +
      (s.catatanBuka ? '<div class="alert alert-info"><i data-icon="info"></i><div><p style="margin:0">' +
        esc(s.catatanBuka) + '</p></div></div>' : '') +
      '<div class="koreksi-list">' + butir + '</div>',
    foot: '<button class="btn btn-outline" onclick="closeModal()">Tutup</button>' +
      (adaManual
        ? '<button class="btn btn-primary" onclick="simpanKoreksi(\'' + esc(sesiId) + '\')">' +
          svgIcon('check', 18) + ' Simpan Penilaian</button>'
        : '<span class="foot-note">Seluruh butir dinilai otomatis — tidak ada yang perlu dikoreksi manual.</span>')
  });
}

function simpanKoreksi(sesiId) {
  const items = {};
  $$('.skor-input').forEach(function (i) {
    items[i.dataset.soal] = { SesiID: sesiId, SoalID: i.dataset.soal, Skor: Number(i.value) || 0, Feedback: '' };
  });
  $$('.fb-ujian-input').forEach(function (i) {
    if (items[i.dataset.soal]) items[i.dataset.soal].Feedback = i.value;
  });

  const arr = Object.keys(items).map(function (k) { return items[k]; });
  const kosong = $$('.skor-input').filter(function (i) { return i.value === ''; });

  const kirim = function () {
    api('apiNilaiUjianBatch', AppState.sessionToken, arr).then(function (res) {
      showToast(res.message, res.success ? 'success' : 'error');
      if (res.success) { closeModal(); muatHasilUjian(UJIAN_STATE.hasil.ujian.ID); }
    }).catch(function (e) { showToast(e.message, 'error'); });
  };
  ...
  if (kosong.length) {
    konfirmasi(kosong.length + ' butir belum diisi skornya dan akan dinilai 0. Lanjutkan?', kirim,
      { labelYa: 'Ya, Simpan' });
  } else kirim();
}

function unduhHasilUjian() {
  if (typeof XLSX === 'undefined') { showToast('Pustaka Excel belum termuat.', 'error'); return; }
  const h = UJIAN_STATE.hasil;
  const baris = [['NIM', 'Nama', 'Status', 'Nilai Objektif', 'Nilai Uraian', 'Nilai Total', 'Pelanggaran', 'Mulai', 'Selesai']];

  h.sesi.forEach(function (s) {
    baris.push([s.nim, s.nama, s.status,
      s.nilaiObjektif === null ? '' : s.nilaiObjektif,
      s.nilaiSubjektif === null ? '' : s.nilaiSubjektif,
      s.nilaiTotal === null ? '' : s.nilaiTotal,
      s.pelanggaran, fmtTgl(s.mulaiAt, true), s.selesaiAt ? fmtTgl(s.selesaiAt, true) : '']);
  });
  h.belumIkut.forEach(function (s) {
    baris.push([s.nim, s.nama, 'Belum mengerjakan', '', '', '', 0, '', '']);
  });

  const ws = XLSX.utils.aoa_to_sheet(baris);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Hasil Ujian');
  XLSX.writeFile(wb, 'Hasil_' + String(h.ujian.Judul).replace(/[^\w\s-]/g, '').slice(0, 40) + '.xlsx');
}


/* -------------------------------------------------------------------------- */
/* H4. DAFTAR UJIAN SISWA                                                     */
/* -------------------------------------------------------------------------- */

PAGES['siswa-ujian'] = function () {
  return headerHalaman('Ujian', 'Daftar ujian untuk kelas Anda.') +
    '<div id="areaUjianSiswa"><section class="card"><div class="card-body"><div class="skeleton-list">' +
    '<div class="skeleton-row"></div><div class="skeleton-row"></div></div></div></section></div>';
};

PAGE_INIT['siswa-ujian'] = function () {
  api('apiGetUjianSiswa', AppState.sessionToken).then(function (res) {
    const area = $('#areaUjianSiswa');
    if (!area) return;
    if (!res.success) { area.innerHTML = kosongState('alert-circle', 'Gagal memuat', res.message); return; }

    const data = res.data || [];
    if (!data.length) {
      area.innerHTML = '<section class="card"><div class="card-body">' +
        kosongState('notebook-pen', 'Belum ada ujian',
          'Ujian akan muncul di sini setelah dosen menerbitkannya.') + '</div></section>';
      return;
    }
    area.innerHTML = '<div class="grid-cards">' + data.map(kartuUjianSiswa).join('') + '</div>';
    renderIcons(area);
  }).catch(function (e) {
    const area = $('#areaUjianSiswa');
    if (area) area.innerHTML = kosongState('alert-circle', 'Gagal memuat', e.message);
...
function kartuUjianSiswa(u) {
  let status, aksi;

  if (u.statusSaya === 'Diblokir') {
    status = '<span class="badge badge-error">Akses Diblokir</span>';
    aksi = '<div class="alert alert-error" style="margin:0"><i data-icon="ban"></i><div>' +
      '<strong>Akses ujian Anda diblokir</strong>' +
      '<p>Anda terdeteksi meninggalkan halaman ujian ' + u.pelanggaran + ' kali. ' +
      'Hubungi dosen atau tim akademik Anda untuk membuka kembali akses.</p></div></div>';
  } else if (u.statusSaya === 'Selesai') {
    status = '<span class="badge badge-success">Sudah Dikumpulkan</span>';
    aksi = u.nilaiSaya !== undefined
      ? '<div class="nilai-akhir-box"><span>Nilai Anda</span><b>' + u.nilaiSaya + '</b>' +
        '<small>dari total bobot ' + u.TotalBobot + '</small></div>'
      : '<p class="muted" style="margin:0">' + svgIcon('clock', 15) + ' Menunggu penilaian dosen.</p>';
  } else if (u.belumDibuka) {
    status = '<span class="badge badge-neutral">Belum Dibuka</span>';
    aksi = '<p class="muted" style="margin:0">' + svgIcon('clock', 15) +
      ' Ujian dibuka ' + esc(fmtTgl(u.TanggalMulai, true)) + '.</p>';
  } else if (u.sudahTutup) {
    status = '<span class="badge badge-neutral plain">Sudah Ditutup</span>';
    aksi = '<p class="muted" style="margin:0">Ujian ditutup pada ' + esc(fmtTgl(u.TanggalSelesai, true)) + '.</p>';
  } else {
    const lanjutan = u.statusSaya === 'Berlangsung';
    status = lanjutan
      ? '<span class="badge badge-info">Sedang Berjalan</span>'
      : '<span class="badge badge-success">Siap Dikerjakan</span>';
    aksi = '<button class="btn btn-primary btn-block" onclick="mulaiUjian(\'' + esc(u.ID) + '\',\'' + esc(u.Judul) + '\')">' +
      svgIcon(lanjutan ? 'play' : 'notebook-pen', 18) + ' ' +
      (lanjutan ? 'Lanjutkan Ujian' : 'Mulai Ujian') + '</button>';
  }

  return '<article class="card ujian-card">' +
    '<div class="ujian-card-head"><div><h3>' + esc(u.Judul) + '</h3>' +
    '<p class="muted">' + esc(namaMapel(u.MapelID)) + ' · ' + esc(u.Jenis) + '</p></div>' + status + '</div>' +
    (u.Deskripsi ? '<p class="ujian-desk">' + esc(u.Deskripsi) + '</p>' : '') +
    '<div class="ujian-meta">' +
      '<span>' + svgIcon('list-checks', 15) + ' ' + u.jumlahSoal + ' soal</span>' +
      '<span>' + svgIcon('timer', 15) + ' ' + u.DurasiMenit + ' menit</span>' +
      '<span>' + svgIcon('star', 15) + ' bobot ' + u.TotalBobot + '</span>' +
    '</div>' +
    (u.TanggalSelesai && !u.sudahTutup
      ? '<p class="ujian-jendela">' + svgIcon('clock', 14) + ' Ditutup ' + esc(fmtTgl(u.TanggalSelesai, true)) + '</p>'
      : '') +
    '<div class="ujian-card-foot single">' + aksi + '</div></article>';
}


/* -------------------------------------------------------------------------- */
/* H5. HALAMAN PENGERJAAN UJIAN                                               */
/* -------------------------------------------------------------------------- */

function mulaiUjian(ujianId, judul) {
  konfirmasi(
    'Anda akan memulai "' + judul + '". Waktu berjalan sejak tombol ini ditekan dan tidak dapat dijeda.\n\n' +
    'Selama ujian berlangsung, berpindah tab atau memperkecil jendela akan tercatat sebagai pelanggaran.',
    function () {
      api('apiMulaiUjian', AppState.sessionToken, ujianId).then(function (res) {
        if (!res.success) {
          showToast(res.message, 'error');
          if (res.data && (res.data.selesai || res.data.diblokir)) navigateTo('siswa-ujian');
          return;
        }
        UJIAN_STATE.sesi = res.data;
        UJIAN_STATE.jawabanTertunda = {};
        UJIAN_STATE.terkunci = false;
        navigateTo('siswa-ujian-kerjakan');
      }).catch(function (e) { showToast(e.message, 'error'); });
    },
    { labelYa: 'Saya Mengerti, Mulai' }
  );
}

PAGES['siswa-ujian-kerjakan'] = function () {
  const s = UJIAN_STATE.sesi;
  if (!s) return kosongState('alert-circle', 'Sesi ujian tidak ditemukan',
    'Kembali ke daftar ujian lalu mulai lagi.',
    '<button class="btn btn-primary" onclick="navigateTo(\'siswa-ujian\')">Ke Daftar Ujian</button>');

  const soal = s.soal.map(kartuSoalUjian).join('');
  const terjawab = s.soal.filter(function (x) { return x.jawabanSaya !== '' || x.fileID; }).length;

  return '<div class="ujian-runtime">' +
    '<div class="ujian-bar">' +
      '<div class="ujian-bar-kiri">' +
        '<b>' + esc(s.ujian.Judul) + '</b>' +
        '<small>' + esc(namaMapel(s.ujian.MapelID)) + ' · ' + s.soal.length + ' soal</small>' +
      '</div>' +
      '<div class="ujian-bar-kanan">' +
        '<div class="ujian-progres"><span id="ujianTerjawab">' + terjawab + '</span>/' + s.soal.length + ' terjawab</div>' +
        '<div class="ujian-timer" id="ujianTimer">--:--</div>' +
      '</div>' +
    '</div>' +

    '<div class="ujian-peringatan" id="ujianPeringatan" hidden></div>' +

    (s.ujian.Deskripsi
      ? '<div class="alert alert-info"><i data-icon="info"></i><div>' +
        '<strong>Petunjuk</strong><p>' + esc(s.ujian.Deskripsi) + '</p></div></div>'
      : '') +

    '<div class="alert alert-warn"><i data-icon="shield"></i><div>' +
      '<strong>Halaman ini diawasi</strong>' +
      '<p>Berpindah tab atau memperkecil jendela tercatat sebagai pelanggaran. ' +
      'Setelah ' + s.maksPelanggaran + ' kali, akses ujian Anda akan diblokir dan hanya dapat dibuka oleh dosen. ' +
      'Jawaban tersimpan otomatis setiap kali Anda mengisi.</p></div></div>' +

    '<div class="soal-list" id="daftarSoalUjian">' + soal + '</div>' +

    '<div class="ujian-submit">' +
      '<div><b id="ringkasJawab">' + terjawab + ' dari ' + s.soal.length + ' soal terjawab</b>' +
      '<small class="muted">Periksa kembali sebelum mengumpulkan. Jawaban tidak dapat diubah setelah dikumpulkan.</small></div>' +
      '<button class="btn btn-primary btn-lg" id="btnKumpulUjian" onclick="kumpulkanUjian()">' +
      svgIcon('send', 20) + ' Kumpulkan Ujian</button>' +
    '</div></div>';
};

function kartuSoalUjian(s) {
  let input;

  if (s.Jenis === 'PG') {
    input = '<div class="opsi-jawab">' + (s.Opsi || []).map(function (o, i) {
      const dipilih = String(s.jawabanSaya) === String(i);
      return '<label class="opsi-jawab-item' + (dipilih ? ' aktif' : '') + '">' +
        '<input type="radio" name="soal_' + esc(s.SoalID) + '" value="' + i + '"' + (dipilih ? ' checked' : '') + ' ' +
        'onchange="jawabSoal(\'' + esc(s.SoalID) + '\', this.value)">' +
        '<span class="opsi-jawab-huruf">' + String.fromCharCode(65 + i) + '</span>' +
        '<span class="opsi-jawab-teks">' + esc(o) + '</span></label>';
    }).join('') + '</div>';

  } else if (s.Jenis === 'Uraian') {
    input = '<textarea class="textarea jawab-uraian" rows="6" data-soal="' + esc(s.SoalID) + '" ' +
      'placeholder="Tulis jawaban Anda di sini…" ' +
      'oninput="jawabUraian(\'' + esc(s.SoalID) + '\', this.value)">' + esc(s.jawabanSaya || '') + '</textarea>' +
      '<div class="jawab-status" id="status_' + esc(s.SoalID) + '"></div>';

  } else {
    input = '<div class="jawab-upload">' +
      (s.fileID
        ? '<div class="berkas-terunggah">' + svgIcon('file-check', 18) +
          '<span>' + esc(s.fileNama || 'Berkas terunggah') + '</span>' +
          '<small class="muted">Mengunggah berkas baru akan menggantikan yang ini.</small></div>'
        : '') +
      '<input class="input" type="file" id="file_' + esc(s.SoalID) + '" ' +
      'onchange="unggahJawabanBerkas(\'' + esc(s.SoalID) + '\')">' +
      '<p class="hint">Maksimal 2 MB. Format bebas — PDF, gambar, atau dokumen.</p>' +
      '<div class="jawab-status" id="status_' + esc(s.SoalID) + '"></div></div>';
  }

  return '<article class="soal-kartu" id="soal_' + esc(s.SoalID) + '">' +
    '<div class="soal-head"><span class="soal-nomor">' + s.Nomor + '</span>' +
    badgeJenisSoal(s.Jenis) + '<span class="muted">bobot ' + s.Bobot + '</span></div>' +
    '<p class="soal-teks">' + esc(s.Pertanyaan) + '</p>' + input + '</article>';
}

PAGE_INIT['siswa-ujian-kerjakan'] = function () {
  const s = UJIAN_STATE.sesi;
  if (!s) return;
  mulaiTimerUjian();
  pasangPengawasUjian();
...
function mulaiTimerUjian() {
  ...
  /* Selisih jam peramban terhadap jam server dihitung sekali di awal, lalu
     dipakai untuk mengoreksi setiap tampilan. Siswa yang memajukan jam
     komputernya tidak mendapat tambahan waktu — batas sesungguhnya tetap
     dijaga server saat pengumpulan. */
  const selisih = new Date(s.serverTime).getTime() - Date.now();
  const batas = new Date(s.batasAt).getTime();

  const tick = function () {
    const el = $('#ujianTimer');
    if (!el) { hentikanTimerUjian(); return; }

    const sisa = Math.max(0, Math.floor((batas - (Date.now() + selisih)) / 1000));
    const m = Math.floor(sisa / 60), d = sisa % 60;
    el.textContent = String(m).padStart(2, '0') + ':' + String(d).padStart(2, '0');

    el.classList.toggle('kritis', sisa <= 60);
    el.classList.toggle('waspada', sisa > 60 && sisa <= 300);

    if (sisa === 300) showToast('Waktu tersisa 5 menit.', 'warning');
    if (sisa === 60)  showToast('Waktu tersisa 1 menit. Segera kumpulkan.', 'warning');

    if (sisa <= 0) {
      hentikanTimerUjian();
      showToast('Waktu habis — jawaban Anda dikumpulkan otomatis.', 'warning');
      kumpulkanUjian(true);
    }
  };

  tick();
  UJIAN_STATE.timer = setInterval(tick, 1000);
}

function hentikanTimerUjian() {
  if (UJIAN_STATE.timer) { clearInterval(UJIAN_STATE.timer); UJIAN_STATE.timer = null; }
}

/* ---------- Pengawasan kecurangan ---------- */

/**
 * Memasang pengawasan halaman ujian.
 *
 * Dua peristiwa dipantau, dan keduanya dihitung sebagai SATU pelanggaran
 * berkat jeda peredam di bawah: `visibilitychange` (pindah tab, minimize,
 * kunci layar) dan `blur` (pindah ke jendela aplikasi lain). Sebagian
 * peramban memicu keduanya sekaligus untuk satu tindakan yang sama —
 * tanpa peredam, satu kali pindah tab akan langsung memblokir siswa.
 */
function pasangPengawasUjian() {
  lepasPengawasUjian();

  let jedaTerakhir = 0;
  const JEDA_PEREDAM_MS = 1500;

  const laporkan = function (jenis) {
    if (UJIAN_STATE.terkunci) return;
    const sekarang = Date.now();
    if (sekarang - jedaTerakhir < JEDA_PEREDAM_MS) return;
    jedaTerakhir = sekarang;

    const sesi = UJIAN_STATE.sesi;
    if (!sesi) return;

    api('apiCatatPelanggaran', AppState.sessionToken, sesi.sesiID, jenis).then(function (res) {
      if (!res.success) return;
      const d = res.data || {};
      if (d.diblokir) {
        UJIAN_STATE.terkunci = true;
        hentikanTimerUjian();
        lepasPengawasUjian();
        tampilkanLayarDiblokir(res.message);
      } else {
        tampilkanPeringatanUjian(res.message, d.pelanggaran, d.maks);
      }
    }).catch(function () { /* jaringan putus sesaat — pelanggaran diabaikan */ });
  };

  const onVisibility = function () {
    if (document.visibilityState === 'hidden') laporkan('pindah tab / jendela disembunyikan');
  };
  const onBlur = function () { laporkan('jendela ujian kehilangan fokus'); };

  /* Klik kanan dan salin-tempel diblokir diam-diam — menghalangi soal disalin
     ke luar, tanpa menghukum siswa yang menekannya karena refleks. */
  const onContextMenu = function (e) { e.preventDefault(); };
  const onCopyPaste = function (e) {
    /* Kolom jawaban tetap boleh disalin-tempel: melarangnya justru menyulitkan
       siswa yang menyusun jawaban panjang, tanpa menambah keamanan apa pun. */
    if (e.target && e.target.classList && e.target.classList.contains('jawab-uraian')) return;
    e.preventDefault();
    showToast('Menyalin isi halaman ujian dinonaktifkan.', 'warning');
  };

  const onSebelumTutup = function (e) {
    if (UJIAN_STATE.terkunci) return;
    e.preventDefault();
    e.returnValue = 'Ujian masih berlangsung. Menutup halaman tidak mengumpulkan jawaban Anda.';
  ...
  window.addEventListener('beforeunload', onSebelumTutup);

  UJIAN_STATE.pengawas = function () {
    document.removeEventListener('visibilitychange', onVisibility);
    window.removeEventListener('blur', onBlur);
    document.removeEventListener('contextmenu', onContextMenu);
    document.removeEventListener('copy', onCopyPaste);
    document.removeEventListener('cut', onCopyPaste);
    window.removeEventListener('beforeunload', onSebelumTutup);
  };
}

function lepasPengawasUjian() {
  if (UJIAN_STATE.pengawas) { UJIAN_STATE.pengawas(); UJIAN_STATE.pengawas = null; }
}

function tampilkanPeringatanUjian(pesan, ke, maks) {
  const el = $('#ujianPeringatan');
  if (!el) return;
  el.hidden = false;
  el.innerHTML = '<div class="peringatan-isi">' + svgIcon('alert-triangle', 24) +
    '<div><strong>Peringatan ' + ke + ' dari ' + maks + '</strong><p>' + esc(pesan) + '</p></div></div>';
  el.classList.remove('getar');
  void el.offsetWidth;          // paksa reflow agar animasi dapat diulang
  el.classList.add('getar');
  renderIcons(el);
}

function tampilkanLayarDiblokir(pesan) {
  const wrap = $('#app-container');
  if (!wrap) return;
  wrap.innerHTML =
    '<div class="blokir-layar">' +
      '<div class="blokir-ikon">' + svgIcon('ban', 56) + '</div>' +
      '<h2>Akses Ujian Diblokir</h2>' +
      '<p>' + esc(pesan) + '</p>' +
      '<div class="blokir-langkah">' +
        '<h4>Langkah selanjutnya</h4>' +
        '<ol>' +
          '<li>Hubungi dosen pengampu atau tim akademik Anda.</li>' +
          '<li>Sampaikan nama, NIM, dan nama ujian ini.</li>' +
          '<li>Setelah akses dibuka, kembali ke menu Ujian dan lanjutkan.</li>' +
        '</ol>' +
      '</div>' +
      '<p class="muted">Jawaban yang sempat Anda simpan tetap tercatat dan tidak hilang.</p>' +
      '<button class="btn btn-primary" onclick="navigateTo(\'siswa-ujian\')">Kembali ke Daftar Ujian</button>' +
    '</div>';
  renderIcons(wrap);
}
...
function jawabSoal(soalId, nilai) {
  const kartu = $('#soal_' + soalId);
  if (kartu) {
    $$('.opsi-jawab-item', kartu).forEach(function (l) {
      l.classList.toggle('aktif', l.querySelector('input').checked);
    });
  }
  kirimJawaban(soalId, nilai);
  perbaruiHitungJawab();
}

/**
 * Uraian disimpan dengan jeda agar tiap ketikan tidak memicu satu permintaan.
 *
 * Teks dimasukkan ke antrean SEKETIKA, sementara pengirimannya ditunda. Dua
 * hal penting bergantung pada pemisahan ini: hitungan "terjawab" langsung
 * benar tanpa menunggu jaringan, dan yang lebih genting — kumpulkanUjian()
 * dapat mengosongkan antrean lebih dulu. Tanpa antrean, menekan "Kumpulkan"
 * kurang dari 900 ms setelah ketikan terakhir akan menutup sesi sebelum
 * kalimat terakhir sempat terkirim, dan kalimat itu hilang tanpa jejak.
 */
function jawabUraian(soalId, teks) {
  const sesi = UJIAN_STATE.sesi;
  if (!sesi || UJIAN_STATE.terkunci) return;

  UJIAN_STATE.jawabanTertunda[soalId] = teks;

  const soal = sesi.soal.filter(function (x) { return String(x.SoalID) === String(soalId); })[0];
  if (soal) soal.jawabanSaya = teks;
  perbaruiHitungJawab();

  const st = $('#status_' + soalId);
  if (st) st.innerHTML = '<span class="muted">Belum tersimpan…</span>';

  jadwalkanSimpanUraian();
}

const jadwalkanSimpanUraian = debounce(function () { kosongkanAntreanUraian(); }, 900);

/**
 * Mengirim seluruh jawaban uraian yang masih mengantre.
 * @return {Promise} selesai setelah semuanya terkirim (atau gagal terkontrol).
 */
function kosongkanAntreanUraian() {
  const antre = UJIAN_STATE.jawabanTertunda;
  const ids = Object.keys(antre);
  if (!ids.length) return Promise.resolve();

  const janji = ids.map(function (id) {
    const teks = antre[id];
    delete antre[id];
    return kirimJawaban(id, teks, true);
  });
  return Promise.all(janji);
}

/** @return {Promise} selalu ter-resolve; kegagalan ditampilkan di baris status. */
function kirimJawaban(soalId, nilai, tampilStatus) {
  const sesi = UJIAN_STATE.sesi;
  if (!sesi || UJIAN_STATE.terkunci) return Promise.resolve();

  const st = $('#status_' + soalId);
  if (tampilStatus && st) st.innerHTML = '<span class="muted">Menyimpan…</span>';

  /* Jawaban lokal diperbarui lebih dulu supaya hitungan "terjawab" langsung
     benar; server menjadi sumber kebenaran saat halaman dimuat ulang. */
  const soal = sesi.soal.filter(function (x) { return String(x.SoalID) === String(soalId); })[0];
  if (soal) soal.jawabanSaya = nilai;

  return apiSilent('apiSimpanJawaban', AppState.sessionToken, sesi.sesiID, soalId, nilai)
    .then(function (res) {
      if (res.success) {
        if (st) {
          st.innerHTML = '<span class="tersimpan">' + svgIcon('check', 14) + ' Tersimpan</span>';
          setTimeout(function () { if (st) st.innerHTML = ''; }, 2200);
        }
        return;
      }
      if (st) st.innerHTML = '<span class="gagal">' + esc(res.message) + '</span>';
      if (res.data && res.data.diblokir) {
        UJIAN_STATE.terkunci = true;
        hentikanTimerUjian(); lepasPengawasUjian();
        tampilkanLayarDiblokir(res.message);
      }
    })
    .catch(function () {
      if (st) st.innerHTML = '<span class="gagal">Gagal menyimpan — periksa koneksi Anda.</span>';
    });
}

function unggahJawabanBerkas(soalId) {
  const input = $('#file_' + soalId);
  const berkas = input.files[0];
  if (!berkas) return;

  const st = $('#status_' + soalId);
  if (berkas.size > 2 * 1024 * 1024) {
    if (st) st.innerHTML = '<span class="gagal">Berkas melebihi 2 MB. Perkecil lalu coba lagi.</span>';
    input.value = '';
    return;
  }
  if (st) st.innerHTML = '<span class="muted">Mengunggah…</span>';

  const reader = new FileReader();
  reader.onload = function (ev) {
    const base64 = String(ev.target.result).split(',')[1];
    api('apiUploadBerkasUjian', AppState.sessionToken, UJIAN_STATE.sesi.sesiID, soalId,
        { name: berkas.name, mimeType: berkas.type, data: base64 })
      .then(function (res) {
        if (!st) return;
        if (res.success) {
          st.innerHTML = '<span class="tersimpan">' + svgIcon('check', 14) + ' ' + esc(berkas.name) + ' terunggah</span>';
          const soal = UJIAN_STATE.sesi.soal.filter(function (x) { return String(x.SoalID) === String(soalId); })[0];
          if (soal) { soal.fileID = res.data.fileID; soal.fileNama = berkas.name; }
          perbaruiHitungJawab();
        } else {
          st.innerHTML = '<span class="gagal">' + esc(res.message) + '</span>';
        }
      }).catch(function (e) {
        if (st) st.innerHTML = '<span class="gagal">' + esc(e.message) + '</span>';
      });
  };
  reader.readAsDataURL(berkas);
}

function perbaruiHitungJawab() {
  const s = UJIAN_STATE.sesi;
  if (!s) return;
  const n = s.soal.filter(function (x) {
    return (x.jawabanSaya !== '' && x.jawabanSaya !== undefined && x.jawabanSaya !== null) || x.fileID;
  }).length;

  const a = $('#ujianTerjawab'); if (a) a.textContent = n;
  const b = $('#ringkasJawab');  if (b) b.textContent = n + ' dari ' + s.soal.length + ' soal terjawab';
}

function kumpulkanUjian(otomatis) {
  const sesi = UJIAN_STATE.sesi;
  if (!sesi || UJIAN_STATE.terkunci) return;
  ...
  const kirim = function () {
    ...
    const btn = $('#btnKumpulUjian');
    if (btn) { btn.disabled = true; btn.innerHTML = 'Mengirim…'; }

    /* Kosongkan antrean uraian SEBELUM sesi dikunci. Menutup sesi lebih dulu
       akan membuat ketikan terakhir ditolak server dan hilang diam-diam —
       pengumpulan harus menunggu antrean, bukan berlomba dengannya. */
    const antreSelesai = kosongkanAntreanUraian();
    UJIAN_STATE.terkunci = true;      // cegah pengumpulan ganda & jawaban susulan

    antreSelesai.then(function () {
      return api('apiSubmitUjian', AppState.sessionToken, sesi.sesiID);
    }).then(function (res) {
      if (!res.success) {
        UJIAN_STATE.terkunci = false;
        if (btn) { btn.disabled = false; btn.innerHTML = svgIcon('send', 20) + ' Kumpulkan Ujian'; }
        showToast(res.message, 'error');
        return;
      }
      UJIAN_STATE.sesi = null;
      tampilkanUjianSelesai(res.data, res.message);
    }).catch(function (e) {
      UJIAN_STATE.terkunci = false;
      if (btn) { btn.disabled = false; btn.innerHTML = svgIcon('send', 20) + ' Kumpulkan Ujian'; }
      showToast('Gagal mengumpulkan: ' + e.message + '. Jawaban Anda tetap tersimpan — coba lagi.', 'error');
    });
  };

  if (otomatis) { kirim(); return; }

  const belum = sesi.soal.filter(function (x) {
    return (x.jawabanSaya === '' || x.jawabanSaya === undefined || x.jawabanSaya === null) && !x.fileID;
  }).length;

  konfirmasi(
    belum
      ? 'Masih ada ' + belum + ' soal yang belum Anda jawab. Kumpulkan sekarang? Jawaban tidak dapat diubah setelah dikumpulkan.'
      : 'Kumpulkan ujian sekarang? Jawaban tidak dapat diubah setelah dikumpulkan.',
    kirim, { labelYa: 'Ya, Kumpulkan', danger: !!belum }
  );
}

function tampilkanUjianSelesai(data, pesan) {
  const wrap = $('#app-container');
  if (!wrap) return;
  const d = data || {};
  wrap.innerHTML =
    '<div class="ujian-selesai">' +
      '<div class="selesai-ikon">' + svgIcon('check-circle', 56) + '</div>' +
      '<h2>Ujian Terkumpul</h2>' +
      '<p>' + esc(pesan) + '</p>' +
      (d.tampilkanNilai && !d.adaButirManual
        ? '<div class="nilai-akhir-box besar"><span>Nilai Anda</span><b>' + d.nilaiObjektif + '</b>' +
          '<small>dari total bobot ' + d.totalBobot + '</small></div>'
        : '') +
      (d.adaButirManual
        ? '<div class="alert alert-info" style="text-align:left"><i data-icon="clock"></i><div>' +
          '<strong>Sebagian soal dinilai manual</strong>' +
          '<p>Soal uraian dan unggahan berkas diperiksa langsung oleh dosen. ' +
          'Nilai akhir muncul setelah seluruh butir selesai dinilai.</p></div></div>'
        : '') +
      '<button class="btn btn-primary" onclick="navigateTo(\'siswa-ujian\')">Kembali ke Daftar Ujian</button>' +
    '</div>';
  renderIcons(wrap);
...
/**
 * Dipanggil router saat pengguna meninggalkan halaman pengerjaan.
 * Tanpa ini, pengawasan dan timer akan terus berjalan di halaman lain dan
 * siswa akan dituduh melanggar padahal ujiannya sudah ditutup.
 */
function bersihkanUjianJikaAda(halamanBaru) {
  if (halamanBaru === 'siswa-ujian-kerjakan') return;
  hentikanTimerUjian();
  lepasPengawasUjian();
}
...
/* -------------------------------------------------------------------------- */
/* H6. KARTU QR & PENGATURAN METODE ABSENSI                                   */
/* -------------------------------------------------------------------------- */

PAGES['kartu-qr'] = function () {
  return AppState.user.peran === 'Siswa' ? halamanKartuQRSiswa() : halamanKartuQRPengelola();
};

PAGE_INIT['kartu-qr'] = function () {
  if (AppState.user.peran === 'Siswa') muatKartuQRSaya();
  else {
    const sel = $('#kelasKartuQR');
    if (sel && sel.value) muatKartuQRKelas();
  }
};

/* ---------- Tampilan siswa ---------- */

function halamanKartuQRSiswa() {
  return headerHalaman('Kartu QR Saya',
    'Kartu identitas digital Anda. Tunjukkan saat verifikasi kehadiran atau ujian luring.') +
    '<div id="areaKartuQR"><section class="card"><div class="card-body">' +
    '<div class="skeleton-list"><div class="skeleton-row"></div></div></div></section></div>';
}

function muatKartuQRSaya() {
  api('apiGetKartuQR', AppState.sessionToken).then(function (res) {
    const area = $('#areaKartuQR');
    if (!area) return;
    if (!res.success || !res.data) {
      area.innerHTML = kosongState('alert-circle', 'Kartu QR belum tersedia',
        res.message || 'Backend tidak mengirimkan data kartu. Pastikan Modul.gs sudah ditempel ' +
        'lengkap dan proyek Apps Script sudah di-deploy sebagai versi baru.');
      return;
    }

    const d = res.data;
    const aturan = {
      Bebas: ['badge-neutral', 'Bebas — semua metode absensi diperbolehkan'],
      GPS:   ['badge-info',    'Wajib absensi lokasi (GPS)'],
      QR:    ['badge-info',    'Wajib pemindaian kode QR']
    }[d.metodeAbsen] || ['badge-neutral', d.metodeAbsen];

    area.innerHTML =
      '<div class="qr-kartu-wrap">' +
        '<div class="qr-kartu" id="qrKartuCetak">' +
          '<div class="qr-kartu-head">' +
            '<span class="qr-institusi">' + esc(d.institusi) + '</span>' +
            '<span class="qr-label">Kartu Identitas Digital</span>' +
          '</div>' +
          '<div class="qr-kartu-body">' +
            '<div class="qr-gambar" id="qrGambar"></div>' +
            '<div class="qr-identitas">' +
              '<h3>' + esc(d.nama) + '</h3>' +
              '<dl><dt>NIM</dt><dd>' + esc(d.nim) + '</dd>' +
              '<dt>Kelas</dt><dd>' + esc(d.kelas) + '</dd></dl>' +
            '</div>' +
          '</div>' +
          '<div class="qr-kartu-foot"><code>' + esc(d.token) + '</code></div>' +
        '</div>' +

        '<div class="qr-samping">' +
          '<div class="alert ' + (d.metodeAbsen === 'Bebas' ? 'alert-info' : 'alert-warn') + '">' +
            '<i data-icon="' + (d.metodeAbsen === 'GPS' ? 'map-pin' : d.metodeAbsen === 'QR' ? 'qr-code' : 'info') + '"></i><div>' +
            '<strong>Aturan absensi Anda</strong><p>' + esc(aturan[1]) + '</p></div></div>' +
          '<h4>Cara memakai</h4>' +
          '<ol class="qr-panduan">' +
            '<li>Simpan atau cetak kartu ini.</li>' +
            '<li>Tunjukkan saat petugas memverifikasi kehadiran Anda secara luring.</li>' +
            '<li>Untuk absensi harian, tetap gunakan menu <b>Absensi Saya</b>.</li>' +
          '</ol>' +
          '<div class="row">' +
            '<button class="btn btn-outline" onclick="unduhKartuQR()">' + svgIcon('download', 18) + ' Unduh PNG</button>' +
            '<button class="btn btn-outline" onclick="cetakKartuQR()">' + svgIcon('printer', 18) + ' Cetak</button>' +
          '</div>' +
          '<p class="hint">Kartu ini bersifat pribadi. Jangan bagikan gambarnya ke siapa pun — ' +
          'siapa saja yang memilikinya dapat mengaku sebagai Anda. Bila kartu tersebar, minta dosen menerbitkan ulang.</p>' +
        '</div>' +
      '</div>';

    renderIcons(area);
    gambarQR('qrGambar', d.token, 190);
  }).catch(function (e) {
    const area = $('#areaKartuQR');
    if (area) area.innerHTML = kosongState('alert-circle', 'Gagal memuat', e.message);
  });
...
/** Merender QR ke dalam elemen. Memakai pustaka qrcodejs yang sudah dimuat. */
function gambarQR(elId, teks, ukuran) {
  const el = document.getElementById(elId);
  if (!el) return;
  el.innerHTML = '';
  if (typeof QRCode === 'undefined') {
    el.innerHTML = '<p class="muted">Pustaka QR belum termuat. Muat ulang halaman.</p>';
    return;
  }
  try {
    new QRCode(el, {
      text: teks, width: ukuran || 180, height: ukuran || 180,
      colorDark: '#022448', colorLight: '#ffffff',
      /* Koreksi galat tingkat H bertahan sampai ~30% permukaan rusak —
         penting untuk kartu yang dicetak lalu terlipat di dompet. */
      correctLevel: QRCode.CorrectLevel.H
    });
  } catch (err) {
    el.innerHTML = '<p class="muted">QR gagal dibuat: ' + esc(err.message) + '</p>';
  }
}

function unduhKartuQR() {
  const canvas = $('#qrGambar canvas');
  if (!canvas) { showToast('Kartu QR belum siap.', 'warning'); return; }
  const a = document.createElement('a');
  a.download = 'KartuQR_' + String((DB.profil || {}).NIM || 'siswa') + '.png';
  a.href = canvas.toDataURL('image/png');
  a.click();
}

function cetakKartuQR() {
  document.body.classList.add('mode-cetak-qr');
  window.print();
  setTimeout(function () { document.body.classList.remove('mode-cetak-qr'); }, 600);
}

/* ---------- Tampilan pengelola ---------- */

function halamanKartuQRPengelola() {
  return headerHalaman('Kartu QR & Aturan Absensi',
    'Cetak kartu QR sekelas, dan tetapkan metode absensi wajib untuk tiap siswa.') +
    '<section class="card"><div class="card-head"><i data-icon="qr-code"></i><h2>Pilih Kelas</h2></div>' +
    '<div class="card-body"><div class="row">' +
      '<select class="select" id="kelasKartuQR" onchange="muatKartuQRKelas()" style="min-width:260px">' +
      opsiSelect(kelasDiampu(), 'ID', 'Nama', '', '— Pilih kelas —') + '</select>' +
      '<button class="btn btn-outline" onclick="cetakSemuaKartuQR()">' + svgIcon('printer', 18) + ' Cetak Semua Kartu</button>' +
    '</div></div></section>' +
    '<div id="areaKartuKelas"></div>';
}

function muatKartuQRKelas() {
  const kelasId = $('#kelasKartuQR').value;
  const area = $('#areaKartuKelas');
  if (!kelasId) { area.innerHTML = ''; return; }

  area.innerHTML = '<section class="card"><div class="card-body"><div class="skeleton-list">' +
    '<div class="skeleton-row"></div><div class="skeleton-row"></div></div></div></section>';

  api('apiGetKartuQRKelas', AppState.sessionToken, kelasId).then(function (res) {
    if (!res.success || !res.data || !res.data.siswa) {
      area.innerHTML = kosongState('alert-circle', 'Kartu kelas belum tersedia',
        res.message || 'Backend tidak mengirimkan data kelas.');
      return;
    }
    UJIAN_STATE.kartuKelas = res.data;
    gambarKartuKelas();
  }).catch(function (e) {
    area.innerHTML = kosongState('alert-circle', 'Gagal memuat', e.message);
  });
}

function gambarKartuKelas() {
  const area = $('#areaKartuKelas');
  const d = UJIAN_STATE.kartuKelas;
  if (!area || !d) return;

  if (!d.siswa.length) {
    area.innerHTML = '<section class="card"><div class="card-body">' +
      kosongState('graduation-cap', 'Kelas ini belum punya siswa',
        'Tambahkan siswa terlebih dahulu di menu Siswa/Mahasiswa.') + '</div></section>';
    return;
  }

  const hitung = { Bebas: 0, GPS: 0, QR: 0 };
  d.siswa.forEach(function (s) { hitung[s.metodeAbsen] = (hitung[s.metodeAbsen] || 0) + 1; });

  area.innerHTML =
    '<section class="card"><div class="card-head"><i data-icon="shield"></i>' +
      '<h2>Aturan Metode Absensi</h2>' +
      '<div class="head-tools">' +
        '<select class="select" id="metodeMassal" style="width:190px">' +
          '<option value="">Terapkan ke semua…</option>' +
          '<option value="Bebas">Bebas (semua metode)</option>' +
          '<option value="GPS">Wajib GPS</option>' +
          '<option value="QR">Wajib QR</option>' +
        '</select>' +
        '<button class="btn btn-outline btn-sm" onclick="terapkanMetodeMassal()">Terapkan</button>' +
      '</div></div>' +
    '<div class="card-body">' +
      '<div class="row" style="margin-bottom:12px">' +
        '<span class="badge badge-neutral plain">Bebas: <b>' + hitung.Bebas + '</b></span>' +
        '<span class="badge badge-info plain">Wajib GPS: <b>' + hitung.GPS + '</b></span>' +
        '<span class="badge badge-info plain">Wajib QR: <b>' + hitung.QR + '</b></span>' +
      '</div>' +
      '<p class="muted" style="margin-top:0">Aturan berlaku untuk pencatatan <b>kehadiran</b>. ' +
      'Pengajuan Sakit dan Izin dengan lampiran bukti tetap dapat dilakukan siswa mana pun.</p>' +
      '<div class="table-wrap"><table class="tbl"><thead><tr>' +
        '<th>NIM</th><th>Nama</th><th>Metode Absensi Wajib</th><th>Kartu QR</th>' +
      '</tr></thead><tbody>' +
      d.siswa.map(function (s) {
        return '<tr>' +
          '<td>' + esc(s.nim) + '</td>' +
          '<td><div class="person"><span class="avatar">' + esc(inisial(s.nama)) + '</span>' + esc(s.nama) + '</div></td>' +
          '<td><select class="select metode-absen" data-id="' + esc(s.id) + '" style="min-width:170px">' +
            '<option value="Bebas"' + (s.metodeAbsen === 'Bebas' ? ' selected' : '') + '>Bebas</option>' +
            '<option value="GPS"'   + (s.metodeAbsen === 'GPS'   ? ' selected' : '') + '>Wajib GPS</option>' +
            '<option value="QR"'    + (s.metodeAbsen === 'QR'    ? ' selected' : '') + '>Wajib QR</option>' +
          '</select></td>' +
          '<td>' + (s.token
            ? '<button class="btn btn-ghost btn-sm" onclick="lihatKartuSatu(\'' + esc(s.id) + '\')">' +
              svgIcon('qr-code', 16) + ' Lihat</button>' +
              '<button class="btn btn-ghost btn-sm danger" title="Terbitkan ulang (kartu lama jadi tidak berlaku)" ' +
              'onclick="terbitkanUlangQR(\'' + esc(s.id) + '\',\'' + esc(s.nama) + '\')">' + svgIcon('refresh', 16) + '</button>'
            : '<span class="muted">Belum terbit</span>') + '</td>' +
          '</tr>';
      }).join('') +
      '</tbody></table></div>' +
      '<div class="row" style="margin-top:14px;justify-content:flex-end">' +
      '<button class="btn btn-primary" onclick="simpanMetodeAbsen()">' + svgIcon('check', 18) + ' Simpan Aturan</button>' +
      '</div>' +
    '</div></section>' +

    '<section class="card"><div class="card-head"><i data-icon="printer"></i>' +
      '<h2>Pratinjau Kartu <small class="muted">(' + d.siswa.length + ' kartu)</small></h2></div>' +
      '<div class="card-body"><div class="qr-grid" id="qrGridCetak">' +
      d.siswa.map(function (s, i) {
        return '<div class="qr-mini">' +
          '<div class="qr-mini-qr" id="qrmini_' + i + '"></div>' +
          '<b>' + esc(s.nama) + '</b><small>' + esc(s.nim) + '</small>' +
          '<small class="muted">' + esc(d.kelas) + '</small></div>';
      }).join('') +
      '</div></div></section>';

  renderIcons(area);
  /* QR digambar setelah DOM terpasang; dipecah agar tabel besar tidak membekukan UI. */
  d.siswa.forEach(function (s, i) {
    if (s.token) setTimeout(function () { gambarQR('qrmini_' + i, s.token, 104); }, i * 12);
  });
}

function lihatKartuSatu(siswaId) {
  const d = UJIAN_STATE.kartuKelas;
  const s = d.siswa.filter(function (x) { return x.id === siswaId; })[0];
  if (!s) return;
  openModal({
    title: 'Kartu QR — ' + s.nama, size: 'slim',
    body: '<div class="qr-kartu" style="margin:0 auto">' +
      '<div class="qr-kartu-head"><span class="qr-institusi">' + esc(d.institusi) + '</span>' +
      '<span class="qr-label">Kartu Identitas Digital</span></div>' +
      '<div class="qr-kartu-body"><div class="qr-gambar" id="qrModalSatu"></div>' +
      '<div class="qr-identitas"><h3>' + esc(s.nama) + '</h3>' +
      '<dl><dt>NIM</dt><dd>' + esc(s.nim) + '</dd><dt>Kelas</dt><dd>' + esc(d.kelas) + '</dd></dl></div></div>' +
      '<div class="qr-kartu-foot"><code>' + esc(s.token) + '</code></div></div>',
    foot: '<button class="btn btn-outline" onclick="closeModal()">Tutup</button>',
    onOpen: function () { gambarQR('qrModalSatu', s.token, 190); }
  });
}

function terapkanMetodeMassal() {
  const nilai = $('#metodeMassal').value;
  if (!nilai) { showToast('Pilih metode yang ingin diterapkan.', 'warning'); return; }
  $$('.metode-absen').forEach(function (sel) { sel.value = nilai; });
  showToast('Diterapkan ke seluruh baris. Tekan "Simpan Aturan" untuk menyimpan.', 'info');
}

function simpanMetodeAbsen() {
  const daftar = $$('.metode-absen').map(function (sel) {
    return { SiswaID: sel.dataset.id, MetodeAbsen: sel.value };
  });
  if (!daftar.length) return;

  api('apiSetMetodeAbsenBatch', AppState.sessionToken, daftar).then(function (res) {
    showToast(res.message, res.success ? 'success' : 'error');
    if (res.success) muatKartuQRKelas();
  }).catch(function (e) { showToast(e.message, 'error'); });
}

function terbitkanUlangQR(siswaId, nama) {
  konfirmasi('Terbitkan ulang kartu QR untuk ' + nama + '? ' +
             'Kartu lama langsung tidak berlaku — pastikan kartu baru dibagikan kepada yang bersangkutan.',
    function () {
      api('apiTerbitkanUlangQR', AppState.sessionToken, [siswaId]).then(function (res) {
        showToast(res.message, res.success ? 'success' : 'error');
        if (res.success) muatKartuQRKelas();
      }).catch(function (e) { showToast(e.message, 'error'); });
    }, { danger: true, labelYa: 'Ya, Terbitkan Ulang' });
}

function cetakSemuaKartuQR() {
  if (!UJIAN_STATE.kartuKelas || !UJIAN_STATE.kartuKelas.siswa.length) {
    showToast('Pilih kelas terlebih dahulu.', 'warning'); return;
  }
  document.body.classList.add('mode-cetak-grid');
  window.print();
  setTimeout(function () { document.body.classList.remove('mode-cetak-grid'); }, 600);
}
